...
try {
  const options = parsePhase5GateArgs(process.argv.slice(2));
  if (options.help) {
    console.log(phase5GateUsage());
  } else {
    const result = await runPhase5Gate(options, { repoRoot: root });
    console.log(JSON.stringify({
      gate: 'bytecode-vm-phase-5',
