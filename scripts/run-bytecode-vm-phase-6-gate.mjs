...
try {
  const options = parsePhase6GateArgs(process.argv.slice(2));
  if (options.help) {
    console.log(phase6GateUsage());
  } else {
    const result = await runPhase6Gate(options, { repoRoot: root });
    console.log(JSON.stringify({
      gate: 'bytecode-vm-phase-6',
