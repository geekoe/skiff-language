...
import { createPhase4EvidenceRoot } from './bytecode-vm-phase-4-evidence-root.mjs';
import {
  checkPhase4Evidence,
  finalizePhase4Evidence,
} from './bytecode-vm-phase-4-evidence.mjs';
import { writePhase4CommandReceipt } from './bytecode-vm-phase-4-receipts.mjs';

const OUTPUT_ENV = 'SKIFF_BYTECODE_VM_PHASE4_EVIDENCE_DIR';
const COMMIT_ENV = 'SKIFF_BYTECODE_VM_PHASE4_CANDIDATE_COMMIT';
const TREE_ENV = 'SKIFF_BYTECODE_VM_PHASE4_CANDIDATE_TREE';

export function parsePhase4GateArgs(args, { env = process.env } = {}) {
...
export async function runPhase4Gate(options, {
  repoRoot,
  ...
  const evidenceRoot = await createPhase4EvidenceRoot(input.outputDir);
  const candidateSpecs = phase4CandidateSpecs(input.repoRoot);
  const workloadSpecs = phase4WorkloadSpecs(input.repoRoot);
  assertPhase4LaneCoverage(workloadSpecs);
  ...
  const handlers = new Map(['SIGINT', 'SIGTERM'].map((signal) => [signal, () => {
    ...
    abortController.abort(new Error(`Phase 4 Gate interrupted by ${signal}`));
  }]));
  ...
  const manifest = await finalizePhase4Evidence({
    evidenceRoot,
  ...
  try {
    await checkPhase4Evidence(input.outputDir, {
      ...input,
  ...
  async function execute(spec) {
    ...
    await evidenceRoot.assertAll();
    await writePhase4CommandReceipt(evidenceRoot, spec, actualEnv, outcome, {
...
export function phase4GateUsage() {
  return `usage: node scripts/run-bytecode-vm-phase-4-gate.mjs \\
