...
import { openPhase3EvidenceRoot } from './bytecode-vm-phase-3-evidence-root.mjs';
import { phase1ObservationSchemaIdentity } from './bytecode-vm-phase-1-observation-schema.mjs';
import {
  loadAndValidatePhase3CommandReceipts,
  writeJsonExclusive,
} from './bytecode-vm-phase-3-receipts.mjs';

const MANIFEST_NAME = 'manifest.json';

export async function finalizePhase3Evidence({
  evidenceRoot,
  repoRoot,
  expectedCommit,
  expectedTree,
  ...
  const evidenceFiles = await snapshotPhase3EvidenceFiles(evidenceRoot);
  const directoryIdentities = evidenceRoot.identities();
  const manifest = {
    schemaVersion: PHASE3_MANIFEST_SCHEMA,
    request: {
      repoRoot,
    ...
    // Phase 3 adds no observation kinds: the accepted Phase 1 eleven-event
...
export async function checkPhase3Evidence(outputDir, request) {
  const evidenceRoot = await openPhase3EvidenceRoot(outputDir, request.directoryIdentities);
  await evidenceRoot.assertAll();
  const manifest = await readJson(evidenceRoot, MANIFEST_NAME, 'manifest');
  if (manifest?.schemaVersion !== PHASE3_MANIFEST_SCHEMA) {
    throw new Error(`manifest schemaVersion must be ${PHASE3_MANIFEST_SCHEMA}`);
  }
  ...
  if (JSON.stringify(manifest.request) !== JSON.stringify(expectedRequest)) {
    throw new Error('manifest request does not match the Phase 3 Gate invocation');
  }
  const actualFiles = await snapshotPhase3EvidenceFiles(evidenceRoot);
...
async function deriveAssessment({
  ...
  const workloads = phase3WorkloadSpecs(repoRoot);
  assertPhase3LaneCoverage(workloads);
  const specs = [
    ...phase3CandidateSpecs(repoRoot),
    ...workloads,
  ];
  const loaded = await loadAndValidatePhase3CommandReceipts(
    evidenceRoot,
    specs,
    commandEnvironments,
  );
...
export async function snapshotPhase3EvidenceFiles(evidenceRoot) {
  const records = [];
