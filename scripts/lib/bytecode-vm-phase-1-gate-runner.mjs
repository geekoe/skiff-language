...
import { createPhase1EvidenceRoot } from './bytecode-vm-phase-1-evidence-root.mjs';
import {
  checkPhase1Evidence,
  finalizePhase1Evidence,
} from './bytecode-vm-phase-1-evidence.mjs';
import { writePhase1CommandReceipt } from './bytecode-vm-phase-1-receipts.mjs';

const OUTPUT_ENV = 'SKIFF_BYTECODE_VM_PHASE1_EVIDENCE_DIR';
const COMMIT_ENV = 'SKIFF_BYTECODE_VM_PHASE1_CANDIDATE_COMMIT';
const TREE_ENV = 'SKIFF_BYTECODE_VM_PHASE1_CANDIDATE_TREE';

export function parsePhase1GateArgs(args, { env = process.env } = {}) {
...
export async function runPhase1Gate(options, {
  repoRoot,
  ...
  const evidenceRoot = await createPhase1EvidenceRoot(input.outputDir);
  const candidateSpecs = phase1CandidateSpecs(input.repoRoot);
  const workloadSpecs = phase1WorkloadSpecs(input.repoRoot);
  assertPhase1LaneCoverage(workloadSpecs);
  ...
  const handlers = new Map(['SIGINT', 'SIGTERM'].map((signal) => [signal, () => {
    ...
    abortController.abort(new Error(`Phase 1 Gate interrupted by ${signal}`));
  }]));
  ...
  const manifest = await finalizePhase1Evidence({
    evidenceRoot,
  ...
  try {
    await checkPhase1Evidence(input.outputDir, {
      ...input,
  ...
  async function execute(spec) {
    ...
    await evidenceRoot.assertAll();
    await writePhase1CommandReceipt(evidenceRoot, spec, actualEnv, outcome, {
...
export function phase1GateUsage() {
  return `usage: node scripts/run-bytecode-vm-phase-1-gate.mjs \\
