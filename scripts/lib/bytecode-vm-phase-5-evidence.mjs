...
import { openPhase5EvidenceRoot } from './bytecode-vm-phase-5-evidence-root.mjs';
import { phase1ObservationSchemaIdentity } from './bytecode-vm-phase-1-observation-schema.mjs';
import {
  loadAndValidatePhase5CommandReceipts,
  writeJsonExclusive,
} from './bytecode-vm-phase-5-receipts.mjs';

const MANIFEST_NAME = 'manifest.json';

export async function finalizePhase5Evidence({
  evidenceRoot,
  repoRoot,
  expectedCommit,
  expectedTree,
  ...
  const evidenceFiles = await snapshotPhase5EvidenceFiles(evidenceRoot);
  const directoryIdentities = evidenceRoot.identities();
  const manifest = {
    schemaVersion: PHASE5_MANIFEST_SCHEMA,
    request: {
      repoRoot,
    ...
    // Phase 5 adds no parallel observation authority. The accepted Phase 1
    // event schema remains the durable event vocabulary; G5/G6/G8 carry the
    // new resource/pending/stream counters through production proof receipts.
...
export async function checkPhase5Evidence(outputDir, request) {
  const evidenceRoot = await openPhase5EvidenceRoot(outputDir, request.directoryIdentities);
  await evidenceRoot.assertAll();
  const manifest = await readJson(evidenceRoot, MANIFEST_NAME, 'manifest');
  if (manifest?.schemaVersion !== PHASE5_MANIFEST_SCHEMA) {
    throw new Error(`manifest schemaVersion must be ${PHASE5_MANIFEST_SCHEMA}`);
  }
  ...
  if (JSON.stringify(manifest.request) !== JSON.stringify(expectedRequest)) {
    throw new Error('manifest request does not match the Phase 5 r1 Gate invocation');
  }
  const actualFiles = await snapshotPhase5EvidenceFiles(evidenceRoot);
...
async function deriveAssessment({
  ...
  const workloads = phase5WorkloadSpecs(repoRoot);
  assertPhase5LaneCoverage(workloads);
  const specs = [
    ...phase5CandidateSpecs(repoRoot),
    ...workloads,
  ];
  const loaded = await loadAndValidatePhase5CommandReceipts(
    evidenceRoot,
    specs,
    commandEnvironments,
  );
...
export async function snapshotPhase5EvidenceFiles(evidenceRoot) {
  const records = [];
