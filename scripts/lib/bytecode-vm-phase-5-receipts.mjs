...
export async function writePhase5CommandReceipt(evidenceRoot, spec, actualEnv, outcome, {
  stdout = '',
  stderr = '',
  startedAt = null,
  finishedAt = null,
  ...
  const receipt = {
    schemaVersion: PHASE5_COMMAND_SCHEMA,
...
export async function loadAndValidatePhase5CommandReceipts(
  evidenceRoot,
  specs,
  commandEnvironments,
) {
  ...
  for (const spec of specs) {
    ...
    records.set(spec.id, {
      ...
      testSummary: spec.testFormat === null
        ? null
        : parsePhase5TestSummary(spec.testFormat, `${stdout}\n${stderr}`),
    });
  }
  return { records, failures };
}

export function phase5CommandIdentity(spec, actualEnv) {
...
function validateReceipt(receipt, spec, actualEnv, stdout, stderr) {
  const failures = [];
  if (receipt?.schemaVersion !== PHASE5_COMMAND_SCHEMA) {
