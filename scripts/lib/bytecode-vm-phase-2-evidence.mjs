...
import { openPhase2EvidenceRoot } from './bytecode-vm-phase-2-evidence-root.mjs';
import { phase1ObservationSchemaIdentity } from './bytecode-vm-phase-1-observation-schema.mjs';
import {
  loadAndValidatePhase2CommandReceipts,
  writeJsonExclusive,
} from './bytecode-vm-phase-2-receipts.mjs';

const MANIFEST_NAME = 'manifest.json';

export async function finalizePhase2Evidence({
  evidenceRoot,
  repoRoot,
  expectedCommit,
  expectedTree,
  ...
  const evidenceFiles = await snapshotPhase2EvidenceFiles(evidenceRoot);
  const directoryIdentities = evidenceRoot.identities();
  const manifest = {
    schemaVersion: PHASE2_MANIFEST_SCHEMA,
    request: {
      repoRoot,
    ...
    // Phase 2 adds no observation kinds: the accepted Phase 1 eleven-event
    // schema is the recorded observation authority for this epoch.
...
export async function checkPhase2Evidence(outputDir, request) {
  const evidenceRoot = await openPhase2EvidenceRoot(outputDir, request.directoryIdentities);
  await evidenceRoot.assertAll();
  const manifest = await readJson(evidenceRoot, MANIFEST_NAME, 'manifest');
  if (manifest?.schemaVersion !== PHASE2_MANIFEST_SCHEMA) {
    throw new Error(`manifest schemaVersion must be ${PHASE2_MANIFEST_SCHEMA}`);
  }
  ...
  if (JSON.stringify(manifest.request) !== JSON.stringify(expectedRequest)) {
    throw new Error('manifest request does not match the Phase 2 Gate invocation');
  }
  const actualFiles = await snapshotPhase2EvidenceFiles(evidenceRoot);
...
async function deriveAssessment({
  ...
  const workloads = phase2WorkloadSpecs(repoRoot);
  assertPhase2LaneCoverage(workloads);
  const specs = [
    ...phase2CandidateSpecs(repoRoot),
    ...workloads,
  ];
  const loaded = await loadAndValidatePhase2CommandReceipts(
    evidenceRoot,
    specs,
    commandEnvironments,
  );
...
export async function snapshotPhase2EvidenceFiles(evidenceRoot) {
  const records = [];
