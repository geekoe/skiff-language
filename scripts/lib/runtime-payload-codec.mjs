...
export function encodeRuntimePayload(value, schema) {
...
export function decodeRuntimePayload(payloadBytes, schema) {
    ...
    if (!reader.done()) {
        throw new RuntimePayloadCodecError(`runtime payload has ${reader.remaining()} trailing byte(s)`);
    }
    return value;
}
function encodeTyped(writer, value, schema, path) {
    if (schema.nullable) {
        if (value === null) {
            writer.writeU8(0);
            return;
    ...
    if ('oneOf' in schema) {
        if (schema.oneOf.length > 256) {
            throw new RuntimePayloadCodecError(`runtime payload union has ${schema.oneOf.length} branches; maximum is 256`);
        }
        const errors = [];
        ...
        throw new RuntimePayloadCodecError(`runtime payload union value did not match any branch: ${errors.join('; ')}`);
    }
...
function decodeTyped(reader, schema, path) {
    if (schema.nullable) {
        ...
        if (discriminant !== 1) {
            throw new RuntimePayloadCodecError(`runtime payload nullable discriminant must be 0 or 1, got ${discriminant}`);
        }
    ...
    if ('oneOf' in schema) {
        ...
        if (branchSchema === undefined) {
            throw new RuntimePayloadCodecError(`runtime payload union branch ${branch} is out of range`);
        }
...
function encodeObject(writer, value, schema, path) {
    if (!isRecord(value)) {
...
function decodeObject(reader, schema, path) {
...
function encodeAny(writer, value, path) {
...
function decodeAny(reader, path) {
...
function decodeString(reader, path) {
    reader.expectTag(TAG_STRING, path);
    try {
        return reader.readStringRaw();
    }
    catch (error) {
        throw new RuntimePayloadCodecError(`runtime payload string at ${path} is not UTF-8: ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
function decodeNumber(reader, path) {
...
function dateEpochMillis(value, path) {
    const epochMillis = value instanceof Date
        ? value.getTime()
        : typeof value === 'string'
            ? Date.parse(value)
            : typeof value === 'number'
...
function encodeStringEnum(writer, value, literals, path) {
    if (literals.length > 256) {
        throw new RuntimePayloadCodecError(`runtime payload union has ${literals.length} branches; maximum is 256`);
    }
    if (typeof value !== 'string') {
        throw new RuntimePayloadCodecError(`expected string at ${path}`);
    }
...
function decodeStringEnum(reader, literals, path) {
    if (literals.length > 256) {
        throw new RuntimePayloadCodecError(`runtime payload union has ${literals.length} branches; maximum is 256`);
    }
    const branch = reader.readU8();
    ...
    if (literal === undefined) {
        throw new RuntimePayloadCodecError(`runtime payload union branch ${branch} is out of range at ${path}`);
    }
    const value = decodeString(reader, path);
    if (value !== literal) {
...
function withoutNullable(schema) {
...
function bytesFromValue(value, path) {
    if (value instanceof Uint8Array) {
        return value;
    }
    if (typeof value === 'string') {
...
class PayloadReader {
    ...
    constructor(payloadBytes) {
        this.buffer = Buffer.from(payloadBytes.buffer, payloadBytes.byteOffset, payloadBytes.byteLength);
    }
    done() {
    ...
    expectTag(expected, path) {
        const actual = this.readU8();
        if (actual !== expected) {
            throw new RuntimePayloadCodecError(`runtime payload expected tag ${expected}, got ${actual} at ${path}`);
        }
    }
    readU8() {
        this.ensure(1);
