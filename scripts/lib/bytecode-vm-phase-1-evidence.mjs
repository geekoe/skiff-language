...
import { openPhase1EvidenceRoot } from './bytecode-vm-phase-1-evidence-root.mjs';
import {
...
export async function finalizePhase1Evidence({
  evidenceRoot,
  repoRoot,
  expectedCommit,
  expectedTree,
  ...
  const evidenceFiles = await snapshotPhase1EvidenceFiles(evidenceRoot);
  const directoryIdentities = evidenceRoot.identities();
  const manifest = {
    schemaVersion: PHASE1_MANIFEST_SCHEMA,
    request: {
      repoRoot,
...
export async function checkPhase1Evidence(outputDir, request) {
  const evidenceRoot = await openPhase1EvidenceRoot(outputDir, request.directoryIdentities);
  await evidenceRoot.assertAll();
  ...
  if (JSON.stringify(manifest.request) !== JSON.stringify(expectedRequest)) {
    throw new Error('manifest request does not match the Phase 1 Gate invocation');
  }
  const actualFiles = await snapshotPhase1EvidenceFiles(evidenceRoot);
...
async function deriveAssessment({
  ...
  const workloads = phase1WorkloadSpecs(repoRoot);
  assertPhase1LaneCoverage(workloads);
  const specs = [
    ...phase1CandidateSpecs(repoRoot),
    ...workloads,
  ];
  const loaded = await loadAndValidatePhase1CommandReceipts(
    evidenceRoot,
    specs,
    commandEnvironments,
  );
...
export async function snapshotPhase1EvidenceFiles(evidenceRoot) {
  const records = [];
