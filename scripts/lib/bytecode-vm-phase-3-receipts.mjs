...
export async function writePhase3CommandReceipt(evidenceRoot, spec, actualEnv, outcome, {
  stdout = '',
  stderr = '',
  startedAt = null,
  finishedAt = null,
  ...
  const receipt = {
    schemaVersion: PHASE3_COMMAND_SCHEMA,
...
export async function loadAndValidatePhase3CommandReceipts(
  evidenceRoot,
  specs,
  commandEnvironments,
) {
  ...
  for (const spec of specs) {
    ...
    records.set(spec.id, {
      ...
      testSummary: spec.testFormat === null
        ? null
        : parsePhase3TestSummary(spec.testFormat, `${stdout}\n${stderr}`),
    });
  }
  return { records, failures };
}

export function phase3CommandIdentity(spec, actualEnv) {
...
function validateReceipt(receipt, spec, actualEnv, stdout, stderr) {
  const failures = [];
  if (receipt?.schemaVersion !== PHASE3_COMMAND_SCHEMA) {
  ...
  if (status === 'PASS' && spec.testFormat !== null) {
    const summary = parsePhase3TestSummary(spec.testFormat, `${stdout}\n${stderr}`);
