...
export async function writePhase2CommandReceipt(evidenceRoot, spec, actualEnv, outcome, {
  stdout = '',
  stderr = '',
  startedAt = null,
  finishedAt = null,
  ...
  const receipt = {
    schemaVersion: PHASE2_COMMAND_SCHEMA,
...
export async function loadAndValidatePhase2CommandReceipts(
  evidenceRoot,
  specs,
  commandEnvironments,
) {
  ...
  for (const spec of specs) {
    ...
    records.set(spec.id, {
      ...
      testSummary: spec.testFormat === null
        ? null
        : parsePhase2TestSummary(spec.testFormat, `${stdout}\n${stderr}`),
    });
  }
  return { records, failures };
}

export function phase2CommandIdentity(spec, actualEnv) {
...
function validateReceipt(receipt, spec, actualEnv, stdout, stderr) {
  const failures = [];
  if (receipt?.schemaVersion !== PHASE2_COMMAND_SCHEMA) {
  ...
  if (status === 'PASS' && spec.testFormat !== null) {
    const summary = parsePhase2TestSummary(spec.testFormat, `${stdout}\n${stderr}`);
