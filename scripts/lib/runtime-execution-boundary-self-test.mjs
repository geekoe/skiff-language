...
export const RUNTIME_EXECUTION_BOUNDARY_MUTATION_EXPECTATIONS = Object.freeze([
  expectation('remote service dispatch selection', 'remote-boundary-selection'),
  ...
  expectation('second service boundary adapter', 'second-in-process-dispatcher'),
...
function mutationMatrix() {
  return [
    mutation(
      'remote service dispatch selection',
    ...
    mutation(
      ...
      (root) => write(
        ...
        'pub fn dispatch_remote_service() {}\n',
      ),
    ),
    mutation(
    ...
    mutation(
      'second service boundary adapter',
      'second-in-process-dispatcher',
      (root) => append(
        root,
        'runtime/eval/src/assembly_execution/mod.rs',
        [
          '',
          'async fn dispatch_backup_service(context: &Context) {',
          '    dispatch_in_process_boundary(context).await;',
          '}',
          '',
        ].join('\n'),
    ...
    mutation(
      ...
      (root) => replace(
        ...
        [
          ...
          '            .route_or_lazy_load(key)',
          '            .expect("active route")',
        ].join('\n'),
        [
          '        let route_or_lazy_load_similar = 1;',
          '        let _ordinary = "route_or_lazy_load(";',
          '        let _ = (key, route_or_lazy_load_similar);',
          '        unreachable!()',
        ].join('\n'),
      ),
    ),
    mutation(
      'assembly request wire dispatch omitted',
      'host-active-assembly-entry-missing',
      (root) => replace(
        root,
        'runtime/host/src/host/request_entry/assembly_wire.rs',
    ...
    mutation(
      'active assembly route exact field omitted',
  ...
  await Promise.all([
    ...
    write(
      ...
      [
        ...
        '            execute_runtime_http_gateway_request(RuntimeHttpGatewayExecutionInput { target }).await;',
        '            drop(route);',
        '        });',
        '    }',
        '}',
    ...
    write(
      ...
      [
        'impl RuntimeHost {',
        '    fn lookup_active_assembly_request_route(',
        '        &self,',
    ...
    write(
      ...
      [
        'impl RuntimeHost {',
        '    async fn resolve_active_assembly_request_route(',
        '        &self,',
        '        key: &ServiceIngressKey,',
        '    ) -> Result<ActiveAssemblyRoute> {',
        '        self.assembly_admission',
