import {
  ...
  parsePhase6TestSummary,
  PHASE6_COMMAND_SCHEMA,
  sha256,
} from './bytecode-vm-phase-6-contract.mjs';

export async function writePhase6CommandReceipt(evidenceRoot, spec, actualEnv, outcome, {
  stdout = '',
  stderr = '',
  startedAt = null,
  finishedAt = null,
  ...
  const receipt = {
    schemaVersion: PHASE6_COMMAND_SCHEMA,
...
export async function loadAndValidatePhase6CommandReceipts(
  evidenceRoot,
  specs,
  commandEnvironments,
) {
  ...
  for (const spec of specs) {
    ...
    records.set(spec.id, {
      ...
      testSummary: spec.testFormat === null
        ? null
        : parsePhase6TestSummary(spec.testFormat, `${stdout}\n${stderr}`),
    });
  }
  return { records, failures };
}

export function phase6CommandIdentity(spec, actualEnv) {
...
function commandIdentity(spec, actualEnv) {
  const identity = {
    ...
    lanes: [...spec.lanes],
    expectedTests: expectedTestsIdentity(spec),
...
function expectedTestsIdentity(spec) {
...
function validateReceipt(receipt, spec, actualEnv, stdout, stderr) {
  const failures = [];
  if (receipt?.schemaVersion !== PHASE6_COMMAND_SCHEMA) {
  ...
  if (status === 'PASS' && spec.testFormat !== null) {
    const summary = parsePhase6TestSummary(spec.testFormat, `${stdout}\n${stderr}`);
    if (summary?.valid !== true) {
      failures.push(failure('command.test-count', `${spec.id} test summary is not exact and complete`));
    } else if (spec.expectedTests !== undefined && spec.expectedTests !== null
