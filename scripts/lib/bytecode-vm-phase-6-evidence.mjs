...
import { openPhase6EvidenceRoot } from './bytecode-vm-phase-6-evidence-root.mjs';
import { phase1ObservationSchemaIdentity } from './bytecode-vm-phase-1-observation-schema.mjs';
import {
  loadAndValidatePhase6CommandReceipts,
  writeJsonExclusive,
} from './bytecode-vm-phase-6-receipts.mjs';

const MANIFEST_NAME = 'manifest.json';

export async function finalizePhase6Evidence({
  evidenceRoot,
  repoRoot,
  expectedCommit,
  expectedTree,
  ...
  const evidenceFiles = await snapshotPhase6EvidenceFiles(evidenceRoot);
  const directoryIdentities = evidenceRoot.identities();
  const manifest = {
    schemaVersion: PHASE6_MANIFEST_SCHEMA,
    request: {
      repoRoot,
    ...
    provenance: phase6WorkloadProvenance(repoRoot),
    boundedWorkLedger: phase6BoundedWorkLedger(repoRoot),
    ...assessment,
    evidenceFiles,
  };
...
export async function checkPhase6Evidence(outputDir, request) {
  const evidenceRoot = await openPhase6EvidenceRoot(outputDir, request.directoryIdentities);
  await evidenceRoot.assertAll();
  const manifest = await readJson(evidenceRoot, MANIFEST_NAME, 'manifest');
  if (manifest?.schemaVersion !== PHASE6_MANIFEST_SCHEMA) {
    throw new Error(`manifest schemaVersion must be ${PHASE6_MANIFEST_SCHEMA}`);
  }
  ...
  if (JSON.stringify(manifest.request) !== JSON.stringify(expectedRequest)) {
    throw new Error('manifest request does not match the Phase 6 r1 Gate invocation');
  }
  const actualFiles = await snapshotPhase6EvidenceFiles(evidenceRoot);
  assertFileClosure(manifest.evidenceFiles, actualFiles);
  const provenance = phase6WorkloadProvenance(request.repoRoot);
  if (JSON.stringify(manifest.provenance) !== JSON.stringify(provenance)) {
    throw new Error('manifest provenance did not match the canonical Phase 6 catalog');
  }
  const ledger = phase6BoundedWorkLedger(request.repoRoot);
  if (JSON.stringify(manifest.boundedWorkLedger) !== JSON.stringify(ledger)) {
    throw new Error('manifest bounded-work ledger did not match the canonical obligation map');
  }
  const assessment = await deriveAssessment({
    evidenceRoot,
    repoRoot: request.repoRoot,
...
async function deriveAssessment({
  ...
  const workloads = phase6WorkloadSpecs(repoRoot);
  assertPhase6LaneCoverage(workloads);
  const provenance = phase6WorkloadProvenance(repoRoot);
  assertPhase6ProvenanceCoverage(workloads, provenance);
  assertPhase6BoundedWorkCoverage(workloads, phase6BoundedWorkLedger(repoRoot));
  const specs = [
    ...phase6CandidateSpecs(repoRoot),
    ...workloads,
  ];
  const loaded = await loadAndValidatePhase6CommandReceipts(
    evidenceRoot,
    specs,
    commandEnvironments,
  );
...
function summarizeCommand(spec, record) {
  return {
    id: spec.id,
    lanes: [...spec.lanes],
    expectedTests: expectedTestsState(spec),
...
function expectedTestsState(spec) {
  if (!Object.hasOwn(spec, 'expectedTests')) return { state: 'missing' };
...
export async function snapshotPhase6EvidenceFiles(evidenceRoot) {
  const records = [];
