...
} from './bytecode-vm-phase-5-evidence.mjs';
import { writePhase5CommandReceipt } from './bytecode-vm-phase-5-receipts.mjs';
...
export function parsePhase5GateArgs(args, { env = process.env } = {}) {
...
export async function runPhase5Gate(options, {
  repoRoot,
  ...
  const handlers = new Map(['SIGINT', 'SIGTERM'].map((signal) => [signal, () => {
    ...
    abortController.abort(new Error(`Phase 5 r1 Gate interrupted by ${signal}`));
  }]));
  ...
  try {
    ...
    if (preflightMatches(outcomes, input) && interruptedBy === null) {
      releaseCargoLease = await acquireCargoLease(PHASE5_CARGO_LEASE_DIR);
  ...
  async function execute(spec) {
    ...
    await evidenceRoot.assertAll();
    await writePhase5CommandReceipt(evidenceRoot, spec, actualEnv, outcome, {
...
export async function acquirePhase5CargoLease(
  leaseDir = PHASE5_CARGO_LEASE_DIR,
  ...
  try {
    ...
    if (error?.code === 'EEXIST') {
      throw new Error(`Phase 5 r1 Cargo lease is already held: ${leaseDir}`);
    }
    throw error;
  }
  let released = false;
...
export function phase5GateUsage() {
  return `usage: node scripts/run-bytecode-vm-phase-5-gate.mjs \\
