...
export async function writePhase1CommandReceipt(evidenceRoot, spec, actualEnv, outcome, {
  stdout = '',
  stderr = '',
  startedAt = null,
  finishedAt = null,
  ...
  const receipt = {
    schemaVersion: PHASE1_COMMAND_SCHEMA,
...
export async function loadAndValidatePhase1CommandReceipts(
  evidenceRoot,
  specs,
  commandEnvironments,
) {
  ...
  for (const spec of specs) {
    ...
    records.set(spec.id, {
      ...
      testSummary: spec.testFormat === null
        ? null
        : parsePhase1TestSummary(spec.testFormat, `${stdout}\n${stderr}`),
    });
  }
  return { records, failures };
}

export function phase1CommandIdentity(spec, actualEnv) {
...
function validateReceipt(receipt, spec, actualEnv, stdout, stderr) {
  const failures = [];
  if (receipt?.schemaVersion !== PHASE1_COMMAND_SCHEMA) {
  ...
  if (status === 'PASS' && spec.testFormat !== null) {
    const summary = parsePhase1TestSummary(spec.testFormat, `${stdout}\n${stderr}`);
