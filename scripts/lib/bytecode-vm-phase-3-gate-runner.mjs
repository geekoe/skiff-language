...
import { createPhase3EvidenceRoot } from './bytecode-vm-phase-3-evidence-root.mjs';
import {
  checkPhase3Evidence,
  finalizePhase3Evidence,
} from './bytecode-vm-phase-3-evidence.mjs';
import { writePhase3CommandReceipt } from './bytecode-vm-phase-3-receipts.mjs';

const OUTPUT_ENV = 'SKIFF_BYTECODE_VM_PHASE3_EVIDENCE_DIR';
const COMMIT_ENV = 'SKIFF_BYTECODE_VM_PHASE3_CANDIDATE_COMMIT';
const TREE_ENV = 'SKIFF_BYTECODE_VM_PHASE3_CANDIDATE_TREE';

export function parsePhase3GateArgs(args, { env = process.env } = {}) {
...
export async function runPhase3Gate(options, {
  repoRoot,
  ...
  const evidenceRoot = await createPhase3EvidenceRoot(input.outputDir);
  const candidateSpecs = phase3CandidateSpecs(input.repoRoot);
  const workloadSpecs = phase3WorkloadSpecs(input.repoRoot);
  assertPhase3LaneCoverage(workloadSpecs);
  ...
  const handlers = new Map(['SIGINT', 'SIGTERM'].map((signal) => [signal, () => {
    ...
    abortController.abort(new Error(`Phase 3 Gate interrupted by ${signal}`));
  }]));
  ...
  const manifest = await finalizePhase3Evidence({
    evidenceRoot,
  ...
  try {
    await checkPhase3Evidence(input.outputDir, {
      ...input,
  ...
  async function execute(spec) {
    ...
    await evidenceRoot.assertAll();
    await writePhase3CommandReceipt(evidenceRoot, spec, actualEnv, outcome, {
...
export function phase3GateUsage() {
  return `usage: node scripts/run-bytecode-vm-phase-3-gate.mjs \\
