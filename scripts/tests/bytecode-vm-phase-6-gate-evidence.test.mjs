...
import { checkPhase6Evidence } from '../lib/bytecode-vm-phase-6-evidence.mjs';
import {
  COMMIT,
  TREE,
  withPhase6EvidenceBundle,
} from './bytecode-vm-phase-6-gate-fixture.mjs';

test('checker accepts one hundred twenty three receipts on one clean candidate', async () => {
  await withPhase6EvidenceBundle({}, async (bundle) => {
    assert.equal(bundle.manifest.verdict, 'PASS');
    assert.deepEqual(bundle.manifest.counts.commands, { total: 123, passed: 123, failed: 0 });
...
for (const fixture of [
  ...
  {
    name: 'missing Phase 6 scenario receipt',
    options: { missingId: 'p6-service-matrix' },
    code: 'command.missing',
  },
  {
    name: 'missing inherited regression receipt',
    options: { missingId: 'phase-5-regression-phase-5-gate-self-tests' },
    code: 'command.missing',
  },
  {
    name: 'interrupted Phase 6 matrix command',
    options: { interruptedId: 'p6-service-matrix' },
    code: 'command.interrupted',
  },
]) {
  test(`checker derives FAIL for ${fixture.name}`, async () => {
    await withPhase6EvidenceBundle(fixture.options, async (bundle) => {
...
for (const fixture of [
  ...
  test(`checker rejects ${fixture.name} Node completion`, async () => {
    await withPhase6EvidenceBundle({ nodeCounts: fixture.nodeCounts }, async (bundle) => {
      assert.equal(bundle.manifest.verdict, 'FAIL');
...
test('checker rejects tampered evidence and every internal symlink', async () => {
  await withPhase6EvidenceBundle({}, async (bundle) => {
    await writeFile(
      join(bundle.outputDir, 'commands', 'phase-6-gate-self-tests.stdout.log'),
      'tampered\n',
    );
...
function check(bundle) {
  return checkPhase6Evidence(bundle.outputDir, {
