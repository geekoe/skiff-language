...
test('checker accepts exactly ninety-two receipts on one clean candidate', async () => {
  await withPhase5EvidenceBundle({}, async (bundle) => {
    assert.equal(bundle.manifest.verdict, 'PASS');
    assert.deepEqual(bundle.manifest.counts.commands, { total: 92, passed: 92, failed: 0 });
