...
test('checker accepts exactly one hundred nine receipts on one clean candidate', async () => {
  await withPhase5EvidenceBundle({}, async (bundle) => {
    assert.equal(bundle.manifest.verdict, 'PASS');
    assert.deepEqual(bundle.manifest.counts.commands, { total: 109, passed: 109, failed: 0 });
