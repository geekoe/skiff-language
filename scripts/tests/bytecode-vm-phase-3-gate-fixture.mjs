...
import { finalizePhase3Evidence } from '../lib/bytecode-vm-phase-3-evidence.mjs';
import { createPhase3EvidenceRoot } from '../lib/bytecode-vm-phase-3-evidence-root.mjs';
import { writePhase3CommandReceipt } from '../lib/bytecode-vm-phase-3-receipts.mjs';

export const COMMIT = 'a'.repeat(40);
export const TREE = 'b'.repeat(40);

export async function withPhase3EvidenceBundle(options, assertion) {
  const created = await mkdtemp(join(tmpdir(), 'skiff-phase3-gate-test-'));
  const temp = await realpath(created);
  const repoRoot = join(temp, 'repo');
  const outputDir = join(temp, 'evidence');
  try {
    await mkdir(repoRoot);
    const evidenceRoot = await createPhase3EvidenceRoot(outputDir);
    const candidateSpecs = phase3CandidateSpecs(repoRoot);
    const workloadSpecs = phase3WorkloadSpecs(repoRoot);
    const commandEnvironment = snapshotCommandEnvironment({
      PATH: '/usr/bin:/bin',
      PHASE3_BOUND_ENV: 'before-execution',
    });
    ...
    for (const spec of [...candidateSpecs, ...workloadSpecs]) {
      ...
      const actualEnv = spec.id === options?.environmentDriftId
        ? snapshotCommandEnvironment({ ...commandEnvironment, PHASE3_BOUND_ENV: 'drifted' })
        : commandEnvironment;
      await writePhase3CommandReceipt(evidenceRoot, spec, actualEnv, interrupted ? {
