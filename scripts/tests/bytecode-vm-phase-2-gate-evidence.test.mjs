...
import { checkPhase2Evidence } from '../lib/bytecode-vm-phase-2-evidence.mjs';
import {
  COMMIT,
  TREE,
  withPhase2EvidenceBundle,
} from './bytecode-vm-phase-2-gate-fixture.mjs';

test('checker accepts exactly thirty-three receipts on one clean candidate', async () => {
  await withPhase2EvidenceBundle({}, async (bundle) => {
    assert.equal(bundle.manifest.verdict, 'PASS');
    assert.deepEqual(bundle.manifest.counts.commands, { total: 33, passed: 33, failed: 0 });
...
for (const fixture of [
  ...
  test(`checker derives FAIL for ${fixture.name}`, async () => {
    await withPhase2EvidenceBundle(fixture.options, async (bundle) => {
...
for (const fixture of [
  ...
  test(`checker rejects ${fixture.name} Node completion`, async () => {
    await withPhase2EvidenceBundle({ nodeCounts: fixture.nodeCounts }, async (bundle) => {
      assert.equal(bundle.manifest.verdict, 'FAIL');
...
test('checker rejects a command log changed after evidence closure', async () => {
  await withPhase2EvidenceBundle({}, async (bundle) => {
    await writeFile(
      join(bundle.outputDir, 'commands', 'phase-1-regression-k0a-compiler-admission.stdout.log'),
      'tampered\n',
    );
...
test('checker rejects actual child environment drift from the bound snapshot', async () => {
  await withPhase2EvidenceBundle({ environmentDriftId: 'phase-2-vcp-production-composition' }, async (bundle) => {
    assert.equal(bundle.manifest.verdict, 'FAIL');
...
function check(bundle) {
  return checkPhase2Evidence(bundle.outputDir, {
