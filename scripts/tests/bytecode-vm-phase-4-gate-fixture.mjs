...
import { finalizePhase4Evidence } from '../lib/bytecode-vm-phase-4-evidence.mjs';
import { createPhase4EvidenceRoot } from '../lib/bytecode-vm-phase-4-evidence-root.mjs';
import { writePhase4CommandReceipt } from '../lib/bytecode-vm-phase-4-receipts.mjs';

export const COMMIT = 'a'.repeat(40);
export const TREE = 'b'.repeat(40);

export async function withPhase4EvidenceBundle(options, assertion) {
  const created = await mkdtemp(join(tmpdir(), 'skiff-phase4-gate-test-'));
  const temp = await realpath(created);
  const repoRoot = join(temp, 'repo');
  const outputDir = join(temp, 'evidence');
  try {
    await mkdir(repoRoot);
    const evidenceRoot = await createPhase4EvidenceRoot(outputDir);
    const candidateSpecs = phase4CandidateSpecs(repoRoot);
    const workloadSpecs = phase4WorkloadSpecs(repoRoot);
    const commandEnvironment = snapshotCommandEnvironment({
      PATH: '/usr/bin:/bin',
      PHASE4_BOUND_ENV: 'before-execution',
    });
    ...
    for (const spec of [...candidateSpecs, ...workloadSpecs]) {
      ...
      const actualEnv = spec.id === options?.environmentDriftId
        ? snapshotCommandEnvironment({ ...commandEnvironment, PHASE4_BOUND_ENV: 'drifted' })
        : commandEnvironment;
      await writePhase4CommandReceipt(evidenceRoot, spec, actualEnv, interrupted ? {
