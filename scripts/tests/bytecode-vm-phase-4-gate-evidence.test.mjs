...
import { checkPhase4Evidence } from '../lib/bytecode-vm-phase-4-evidence.mjs';
import {
  COMMIT,
  TREE,
...
for (const fixture of [
  ...
  {
    name: 'missing Phase 3 regression receipt',
    options: { missingId: 'phase-3-regression-phase-2-regression-phase-2-vcp-production-composition' },
    code: 'command.missing',
  },
  {
    name: 'missing Phase 4 scenario receipt',
    options: { missingId: 'phase-4-negative-session-disconnect' },
    code: 'command.missing',
  },
  { name: 'missing fresh receipt', options: { missingId: 'fresh-head' }, code: 'command.missing' },
  {
    name: 'interrupted VCP command',
    options: { interruptedId: 'phase-4-vcp-production-composition' },
  ...
  test(`checker derives FAIL for ${fixture.name}`, async () => {
    await withPhase4EvidenceBundle(fixture.options, async (bundle) => {
...
for (const fixture of [
  ...
  test(`checker rejects ${fixture.name} Node completion`, async () => {
    await withPhase4EvidenceBundle({ nodeCounts: fixture.nodeCounts }, async (bundle) => {
      assert.equal(bundle.manifest.verdict, 'FAIL');
...
test('checker rejects a command log changed after evidence closure', async () => {
  await withPhase4EvidenceBundle({}, async (bundle) => {
    await writeFile(
      join(bundle.outputDir, 'commands', 'phase-3-regression-phase-3-vcp-production-composition.stdout.log'),
      'tampered\n',
    );
...
test('checker rejects actual child environment drift from the bound snapshot', async () => {
  await withPhase4EvidenceBundle({ environmentDriftId: 'phase-4-vcp-production-composition' }, async (bundle) => {
    assert.equal(bundle.manifest.verdict, 'FAIL');
...
function check(bundle) {
  return checkPhase4Evidence(bundle.outputDir, {
