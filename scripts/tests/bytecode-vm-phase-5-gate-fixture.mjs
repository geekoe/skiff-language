...
import { finalizePhase5Evidence } from '../lib/bytecode-vm-phase-5-evidence.mjs';
import { createPhase5EvidenceRoot } from '../lib/bytecode-vm-phase-5-evidence-root.mjs';
import { writePhase5CommandReceipt } from '../lib/bytecode-vm-phase-5-receipts.mjs';

export const COMMIT = 'a'.repeat(40);
export const TREE = 'b'.repeat(40);

export async function withPhase5EvidenceBundle(options, assertion) {
  const created = await mkdtemp(join(tmpdir(), 'skiff-phase5-gate-test-'));
  const temp = await realpath(created);
  const repoRoot = join(temp, 'repo');
  const outputDir = join(temp, 'evidence');
  try {
    await mkdir(repoRoot);
    const evidenceRoot = await createPhase5EvidenceRoot(outputDir);
    const candidateSpecs = phase5CandidateSpecs(repoRoot);
    const workloadSpecs = phase5WorkloadSpecs(repoRoot);
    const commandEnvironment = snapshotCommandEnvironment({
      PATH: '/usr/bin:/bin',
      PHASE5_BOUND_ENV: 'before-execution',
    });
    ...
    for (const spec of [...candidateSpecs, ...workloadSpecs]) {
      ...
      const actualEnv = spec.id === options?.environmentDriftId
        ? snapshotCommandEnvironment({ ...commandEnvironment, PHASE5_BOUND_ENV: 'drifted' })
        : commandEnvironment;
      await writePhase5CommandReceipt(evidenceRoot, spec, actualEnv, interrupted ? {
...
function workloadOutput(spec, options) {
  ...
  if (spec.testFormat === 'rust-suite') {
    return rust({
      ...options?.rustCounts,
      passed: options?.rustCounts?.passed ?? spec.expectedTests ?? 3,
    });
  }
  return '';
}
