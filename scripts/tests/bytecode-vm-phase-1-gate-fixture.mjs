...
import { finalizePhase1Evidence } from '../lib/bytecode-vm-phase-1-evidence.mjs';
import { createPhase1EvidenceRoot } from '../lib/bytecode-vm-phase-1-evidence-root.mjs';
import { writePhase1CommandReceipt } from '../lib/bytecode-vm-phase-1-receipts.mjs';

export const COMMIT = 'a'.repeat(40);
export const TREE = 'b'.repeat(40);

export async function withPhase1EvidenceBundle(options, assertion) {
  const created = await mkdtemp(join(tmpdir(), 'skiff-phase1-gate-test-'));
  const temp = await realpath(created);
  const repoRoot = join(temp, 'repo');
  const outputDir = join(temp, 'evidence');
  try {
    await mkdir(repoRoot);
    const evidenceRoot = await createPhase1EvidenceRoot(outputDir);
    const candidateSpecs = phase1CandidateSpecs(repoRoot);
    const workloadSpecs = phase1WorkloadSpecs(repoRoot);
    const commandEnvironment = snapshotCommandEnvironment({
      PATH: '/usr/bin:/bin',
      PHASE1_BOUND_ENV: 'before-execution',
    });
    ...
    for (const spec of [...candidateSpecs, ...workloadSpecs]) {
      ...
      const actualEnv = spec.id === options?.environmentDriftId
        ? snapshotCommandEnvironment({ ...commandEnvironment, PHASE1_BOUND_ENV: 'drifted' })
        : commandEnvironment;
      await writePhase1CommandReceipt(evidenceRoot, spec, actualEnv, interrupted ? {
