...
test('runner receipts all one hundred nine commands and freezes the actual environment', async () => {
  ...
  try {
    ...
    assert.deepEqual(result.manifest.counts.commands, { total: 109, passed: 109, failed: 0 });
    ...
    assert.equal(phase5.length, 54);
    assert.equal(regression.every(({ status }) => status === 'PASS'), true);
    assert.equal(observed.length, 109);
...
test('runner records every later receipt after one executable expected-red workload', async () => {
  ...
  try {
    ...
    assert.equal(result.checkerError, null);
    assert.deepEqual(result.manifest.counts.commands, { total: 109, passed: 108, failed: 1 });
    assert.equal(observed.length, 109, 'one red workload must not truncate the Gate matrix');
    assert.equal(
