...
try {
  const options = parsePhase4GateArgs(process.argv.slice(2));
  if (options.help) {
    console.log(phase4GateUsage());
  } else {
    const result = await runPhase4Gate(options, { repoRoot: root });
    console.log(JSON.stringify({
      gate: 'bytecode-vm-phase-4',
