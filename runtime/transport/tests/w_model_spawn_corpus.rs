//! W-model-spawn corpus gate: the spawn-wire corpus is consumed through the
...
//! golden `frameHex` values are the C-model-spawn frozen bytes and must not
//! change.
//!
//! Frozen contract: `doc/implementation/router-rust-migration-c-model-spawn-contract.md`.
...
mod tests {
    ...
    #[test]
    fn w_model_spawn_corpus_is_frozen() {
        let catalog = catalog();
    ...
    #[test]
    fn w_model_spawn_canonical_requests_roundtrip_byte_exact() {
        let catalog = catalog();
        for name in [
            "spawn.submit.request.function",
            "spawn.submit.request.actorMethod",
    ...
    #[test]
    fn w_model_spawn_response_and_error_roundtrip_byte_exact() {
        let catalog = catalog();
    ...
    #[test]
    fn w_model_spawn_legacy_old_shape_has_no_compatible_reader() {
        let catalog = catalog();
    ...
    #[test]
    fn w_model_spawn_target_kind_and_actor_method_must_agree() {
    ...
    #[test]
    fn w_model_spawn_scenario_names_are_frozen() {
        let mut names = Vec::new();
        for entry in std::fs::read_dir(concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/testdata/spawn-wire/scenarios"
        ))
