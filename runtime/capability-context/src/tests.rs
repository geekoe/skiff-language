...
fn file_capability_error_payload_and_catch_projection_match_public_contract() {
    ...
    let provider = FileCapabilityError::provider_unavailable("svc.account", "no active runtime");
...
fn fixed_service_stream_terminal_keeps_exact_bytes_and_typed_import_facts() {
    ...
    let (caller_build, caller_executable, terminal_site, terminal_stack, service_id, operation_id) =
