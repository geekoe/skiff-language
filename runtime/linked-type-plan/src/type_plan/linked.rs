...
impl RuntimeTypePlanLinkedExt for RuntimeTypePlan {
    ...
    fn from_artifact_type_ref(type_ref: &skiff_artifact_model::TypeRefIr) -> Result<Self> {
        ...
        let node = match type_ref {
            TypeRefIr::Builtin { name, args } => artifact_builtin_node(name, args)?,
    ...
    fn from_linked(type_ref: &LinkedTypeRef, ctx: &PlanContext) -> Result<Self> {
        from_linked_impl(type_ref, ctx)
    }

    fn from_linked_nested_ref(type_ref: &LinkedTypeRef, ctx: &PlanContext) -> Result<Self> {
        from_linked_ref(type_ref, ctx)
    }
}

pub(crate) fn from_artifact_type_ref_in_program_ref(
    ...
    let node = match type_ref {
        TypeRefIr::Builtin { name, args } => artifact_builtin_node_in_program(name, args, ctx)?,
        TypeRefIr::Record { fields } => RuntimeTypeNode::Record {
            fields: fields
                .iter()
                .map(|(name, ty)| {
                    Ok(RuntimeRecordFieldPlan {
                        name: name.clone(),
                        ty: from_artifact_type_ref_in_program_ref(ty, ctx)?,
                        required: !matches!(ty, TypeRefIr::Nullable { .. }),
                        identity: None,
                    })
                })
        ...
        TypeRefIr::Union { items } => RuntimeTypeNode::Union(
            items
                .iter()
                .map(|item| from_artifact_type_ref_in_program_ref(item, ctx))
                .collect::<Result<Vec<_>>>()?,
        ),
        TypeRefIr::Nullable { inner } => {
            RuntimeTypeNode::Nullable(Box::new(from_artifact_type_ref_in_program_ref(inner, ctx)?))
        }
        TypeRefIr::Literal {
            value: LiteralIr::String { value },
        } => RuntimeTypeNode::LiteralString(value.clone()),
        TypeRefIr::PackageSymbol { symbol } => {
            ...
            return from_linked_ref(&linked, ctx);
        }
...
///     descriptor (see [`RuntimeTypePlan::from_linked_ref`]).
...
pub(crate) fn from_linked_impl(
    ...
    match type_ref {
        ...
        LinkedTypeRef::Address { addr } => {
            ...
            from_linked_declaration(declaration, ctx)
        }
        ...
        _ => from_linked_ref(type_ref, ctx),
    }
}
...
///
/// This intentionally uses the resolver semantics of [`RuntimeTypePlan::from_linked_ref`]:
/// `LocalType`, `ServiceSymbol`, and `PackageSymbol` refs resolve here,
/// unlike the top-level [`RuntimeTypePlan::from_linked`] entry point where those refs
/// still report the historical "not linked before execution" artifact
/// error.
pub(crate) fn from_linked_ref(
    type_ref: &LinkedTypeRef,
    ctx: &PlanContext,
) -> Result<RuntimeTypePlan> {
    ...
    let node = match type_ref {
        LinkedTypeRef::Native { name, args } => builtin_node(name, args, ctx)?,
        ...
        LinkedTypeRef::Record { fields } => RuntimeTypeNode::Record {
            fields: fields
                ...
                .map(|(name, field_ty)| {
                    Ok(RuntimeRecordFieldPlan {
                        name: name.clone(),
                        ty: from_linked_ref(field_ty, &ctx.without_substitutions().deeper_by(2))?,
        ...
        LinkedTypeRef::Address { addr } => {
            return resolve_addr_or_bridge(type_ref, addr.clone(), ctx);
        }
        LinkedTypeRef::LocalType { type_index } => {
            ...
            return resolve_addr_or_bridge(type_ref, addr, ctx);
        }
        LinkedTypeRef::PublicationType {
            ...
            match program_publication_type_addr(
                ...
                *type_index,
                ...
                Some(addr) => return resolve_addr_or_bridge(type_ref, addr, ctx),
        ...
        LinkedTypeRef::TypeParam { name } => {
            if let Some(bound) = ctx.substitution(name) {
                return from_linked_substituted(bound, ctx);
            }
...
pub(crate) fn from_linked_substituted(
    bound: &LinkedTypeRef,
    ctx: &PlanContext,
) -> Result<RuntimeTypePlan> {
    let substituted = close_linked_type_ref(bound, ctx.substitutions)?;
    from_linked_impl(&substituted, &ctx.without_substitutions())
}
...
/// unresolved -> Unknown).
pub(crate) fn resolve_addr_or_bridge(
    type_ref: &LinkedTypeRef,
    addr: TypeAddr,
    ctx: &PlanContext,
) -> Result<RuntimeTypePlan> {
    match ctx.program.types.declaration(&addr) {
        ...
        Some(declaration) => from_linked_declaration(declaration, &ctx.deeper_by(1)),
        None => Ok(unknown_plan_for_type_ref(type_ref)),
    }
}

pub(crate) fn from_linked_declaration(
    ...
    let mut plan = from_linked_descriptor(&declaration.descriptor, ctx)?;
...
pub(crate) fn from_linked_descriptor(
    ...
    let node = match descriptor {
        LinkedTypeDescriptor::Record { fields } => RuntimeTypeNode::Record {
            fields: fields
                ...
                .map(|(name, field_ty)| {
                    Ok(RuntimeRecordFieldPlan {
                        name: name.clone(),
                        ty: from_linked_ref(field_ty, &ctx.deeper_by(2))?,
                        required: !matches!(field_ty, LinkedTypeRef::Nullable { .. }),
                        identity: None,
                    })
                })
        ...
        LinkedTypeDescriptor::Representation { representation } => {
            RuntimeTypeNode::Representation {
                ...
                payload: Box::new(from_linked_ref(representation, &ctx.deeper_by(1))?),
            }
        }
        LinkedTypeDescriptor::Alias { target } => {
            RuntimeTypeNode::Alias(Box::new(from_linked_ref(target, &ctx.deeper_by(1))?))
        }
...
pub(crate) fn artifact_builtin_node(
    name: &str,
...
pub(crate) fn artifact_builtin_node_in_program(
    name: &str,
    args: &[skiff_artifact_model::TypeRefIr],
    ctx: &PlanContext<'_>,
) -> Result<RuntimeTypeNode> {
