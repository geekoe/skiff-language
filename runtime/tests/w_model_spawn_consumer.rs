//! M-spawn consumer gate: the `runtime` crate consumes the frozen
//! C-model-spawn corpus (`transport/testdata/spawn-wire/`) through the
//! canonical spawn codec. The legacy old shape has no compatible reader and
...
fn corpus_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("transport/testdata/spawn-wire")
}
...
mod tests {
    ...
    #[test]
    fn runtime_consumer_roundtrips_spawn_wire_corpus_through_canonical_codec() {
        let catalog = catalog();
    ...
    #[test]
    fn runtime_consumer_sees_all_frozen_spawn_scenarios() {
