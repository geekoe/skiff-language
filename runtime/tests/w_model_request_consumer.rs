//! M-request consumer gate: the `runtime` crate consumes the frozen
//! C-model-request corpus directly through the W-model transport codecs.
//!
//! The Runtime side of ordinary request traffic (C-model-request §2)
//! decodes `request.start` / `request.cancel` (RouterToRuntime) and encodes
...
fn corpus_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("transport/testdata/request-wire")
}
...
mod tests {
    ...
    #[test]
    fn runtime_consumer_roundtrips_request_wire_corpus_byte_exact() {
        let catalog = catalog();
        ...
        for (name, entry) in &catalog.frames {
            ...
            let reencoded = match entry.decode_as.as_str() {
                ...
                "RequestCancel" => {
                    ...
                    assert_eq!(
                        header.reason,
                        entry.header["reason"].as_str().expect("reason"),
                        "{name}"
                    );
                    assert!(
            ...
            assert_eq!(
                reencoded, bytes,
                "{name} must roundtrip byte-exact in the Runtime consumer"
            );
        }
    }

    #[test]
    fn runtime_consumer_sees_all_frozen_request_wire_scenarios() {
