//! M-actor consumer gate: the `runtime` crate consumes the frozen
//! C-model-actor corpus (`transport/testdata/actor-wire/`) through the
//! canonical transport codecs. Every one of the 20 frozen actor frames must
...
fn corpus_dir() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("transport/testdata/actor-wire")
}
...
mod tests {
    ...
    #[test]
    fn runtime_consumer_roundtrips_actor_wire_corpus_through_canonical_codecs() {
        let catalog = catalog();
    ...
    #[test]
    fn runtime_consumer_sees_all_frozen_actor_scenarios() {
