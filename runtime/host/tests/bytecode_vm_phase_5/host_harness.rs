...
use skiff_runtime_transport::protocol::{
    ...
    decode_response_start_frame, decode_runtime_capabilities_frame, decode_runtime_health_frame,
    decode_typed_binary_frame, encode_binary_frame, encode_request_cancel_frame,
    ...
    BytecodeRequestTraceFrameHeader, RequestCancelFrameHeader, ResponseEndFrameMetadata,
    ...
    RuntimeRegisteredFrameHeader, TypedEnvelope, ValidatedResponseErrorFrame,
...
use super::{fixture::PublishedFixture, stages::published_positive};

const IO_TIMEOUT: Duration = Duration::from_secs(5);
static NEXT_RUNTIME_HOME: AtomicU64 = AtomicU64::new(0);

/// A deterministic Router-side peer around the public production RuntimeHost.
/// It owns no dispatcher, provider port, resource handle, response event, or
/// VM frame. Requests and cancellations enter only through canonical
...
impl RuntimeHostHarness {
    pub async fn start(prefix: &str) -> Self {
        ...
        let initial = harness
            .next_health_matching("initial zero owner inventory", health_counters_all_zero)
            .await;
        assert!(
            health_counters_all_zero(&initial),
    ...
    pub async fn send_cancel(&mut self, request_id: &str, reason: &str) {
        let frame = encode_request_cancel_frame(&RequestCancelFrameHeader {
            schema_version: RUNTIME_FRAME_SCHEMA_VERSION.to_string(),
            envelope_type: "request.cancel".to_string(),
            request_id: request_id.to_string(),
            reason: reason.to_string(),
        })
        .expect("encode canonical request.cancel");
        self.websocket
            .send(Message::Binary(frame.into()))
            .await
            .expect("send canonical request.cancel");
    }

    pub async fn next_health_matching(
        &mut self,
        context: &str,
    ...
    pub async fn response(&mut self, request_id: &str) -> HostResponse {
        ...
        loop {
            let bytes = next_non_health(&mut self.websocket, deadline, request_id).await;
            let (typed, _) = decode_typed_binary_frame::<TypedEnvelope>(&bytes)
    ...
    pub async fn control_error(&mut self, request_id: &str) -> RuntimeErrorFramePayload {
        let deadline = Instant::now() + IO_TIMEOUT;
        ...
        assert_eq!(header.request_id(), request_id);
        match error {
            ValidatedResponseErrorFrame::Control(error) => error,
            ValidatedResponseErrorFrame::FixedService(_) => {
                panic!("deadline terminal must be a control response.error")
            }
        }
    }
...
async fn next_non_health(
    websocket: &mut WebSocketStream<TcpStream>,
    deadline: Instant,
    request_id: &str,
) -> Vec<u8> {
    loop {
        let bytes = next_binary(websocket, deadline, request_id).await;
        let (typed, _) = decode_typed_binary_frame::<TypedEnvelope>(&bytes)
            .expect("decode RuntimeHost typed frame");
        match typed.envelope_type.as_str() {
