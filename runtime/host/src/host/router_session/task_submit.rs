//! H-task-parent-cut driver-side `task.submit.request` encoder.
//!
//! The Runtime driver owns the production outbound wire for task submits.
//! After the cut it encodes the canonical
//! [`TaskSubmitRequestFrameHeaderV2`] with the closed `callerKind` enum and
//! rejects the legacy shape (`OutboundControlMessage::TaskSubmit` without
...
pub(crate) fn legacy_task_submit_rejected() -> RuntimeError {
    RuntimeError::Protocol {
        target: TASK_SUBMIT_REQUEST_FRAME_TYPE.to_string(),
        message: "task.submit.request without callerKind is not supported (H-task-parent-cut); the legacy shape has no compatible reader"
            .to_string(),
    }
}

pub(crate) fn encode_task_submit_wire_message(
    message: TaskSubmitControlMessage,
) -> Result<Vec<u8>> {
    let header = task_submit_request_header_v2(&message.request, message.caller_kind)?;
    encode_task_submit_request_frame(&header, &message.payload).map_err(|error| {
        RuntimeError::Protocol {
            target: TASK_SUBMIT_REQUEST_FRAME_TYPE.to_string(),
            message: format!("failed to encode canonical task.submit.request: {error}"),
        }
    })
}

fn task_submit_request_header_v2(
    request: &TaskSubmitControlRequest,
    caller_kind: TaskCallerKind,
) -> Result<TaskSubmitRequestFrameHeaderV2> {
    ...
    skiff_artifact_identity::publication_storage_segment(&request.service_id, "service ID")
        .map_err(|_| RuntimeError::Protocol {
            target: TASK_SUBMIT_REQUEST_FRAME_TYPE.to_string(),
            message: "task.submit.request serviceId is not a valid publication service ID"
                .to_string(),
        })?;
    ...
    let target_kind = match request.target_kind.as_str() {
        ...
        other => {
            return Err(RuntimeError::Protocol {
                target: TASK_SUBMIT_REQUEST_FRAME_TYPE.to_string(),
                message: format!(
                    "task.submit.request targetKind {other} is not a closed dispatch target kind"
                ),
            });
        }
    };
    ...
    let actor_method = request
        ...
        .transpose()?;
    Ok(TaskSubmitRequestFrameHeaderV2 {
        ...
        caller_kind: match caller_kind {
            TaskCallerKind::Request => WireTaskCallerKind::Request,
            TaskCallerKind::ActorInvocation => WireTaskCallerKind::ActorInvocation,
        },
        caller_request_id,
        target_kind,
...
fn actor_method_task_target_frame(
    target: &skiff_runtime_capability_context::ActorMethodTaskTargetControl,
) -> Result<TaskActorMethodTargetFrameMetadata> {
    let epoch = target
        .actor_ref
        .epoch()
        .ok_or_else(|| RuntimeError::Protocol {
            target: TASK_SUBMIT_REQUEST_FRAME_TYPE.to_string(),
            message: "actor method dispatch target requires a pinned Actor epoch".to_string(),
        })?;
