...
pub(in crate::host::request_entry) const PHASE2_VCP_FIXTURE_RELATIVE: &str =
    "runtime/host/src/host/request_entry/phase_2_proof_support/fixtures/vcp2-nested-aggregate";
pub(in crate::host::request_entry) const PHASE2_NEGATIVE_FIXTURE_RELATIVE: &str =
    "runtime/host/src/host/request_entry/phase_2_proof_support/fixtures/vcp2-missing-plan";
...
const VCP_INGRESS_PATH: &str = "/phase-2/vcp";
const VCP_REQUEST_BODY: &[u8] = b"1";
...
pub(in crate::host::request_entry) enum Phase2FixtureBuild {
    Published(Phase2PublishedFixture),
    Rejected { error_chain: String },
}

pub(in crate::host::request_entry) struct Phase2PublishedFixture {
...
impl Phase2PublishedFixture {
    /// Builds the fixture through the production compiler/authoring/publication
    /// seam, exactly mirroring `PublishedFixture::build` from Phase 0 (which is
    /// hard-wired to the scalar fixture and therefore cannot be reused here).
    pub(in crate::host::request_entry) fn build(
        ...
        let authoring_receipt = match build_authoring_object(
            ...
            Err(error) => {
                ...
                return Phase2FixtureBuild::Rejected {
    ...
    fn http_header(
        ...
        BytecodeRequestStartFrameHeader {
            ...
            http_request: BytecodeHttpRequestFrameHeader {
                method: "POST".to_string(),
                url: format!("http://phase-2.example.test{}", self.ingress_path),
                path: self.ingress_path.to_string(),
                query: Vec::new(),
                headers: Vec::new(),
            },
    ...
    pub(in crate::host::request_entry) fn canonical_request(
        ...
        let frame = encode_binary_frame(&self.http_header(correlation, mode), self.request_body)
            .expect("encode canonical Phase 2 SKBF request");
        let (header, body) = decode_bytecode_request_start_frame(&frame)
            .expect("production decoder accepts canonical Phase 2 request");
        CanonicalSkbfRequest {
            frame,
            header,
            body,
