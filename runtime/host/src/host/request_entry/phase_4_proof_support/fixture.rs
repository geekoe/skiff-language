...
use skiff_artifact_model::{
    BytecodeArtifactRef, GatewayEntryIdentity, IngressProtocol, IngressSelector, PackageArtifact,
    PackageArtifactRef, ServiceDeployment, ServiceDeploymentRef,
};
use skiff_compiler::{
...
pub(in crate::host::request_entry) const PHASE4_VCP_FIXTURE_RELATIVE: &str =
...
pub(in crate::host::request_entry) enum Phase4FixtureBuild {
    Published(Phase4PublishedFixture),
    Rejected { error_chain: String },
}

pub(in crate::host::request_entry) struct Phase4PublishedFixture {
...
impl Phase4PublishedFixture {
    /// Builds the fixture through the production compiler/authoring/publication
    /// seam, exactly mirroring `Phase3PublishedFixture::build` (which is
    /// hard-wired to the Phase 3 fixtures and therefore cannot be reused here).
    pub(in crate::host::request_entry) fn build(
        ...
        // The Phase 4 fixture imports `std` for the canonical sleep effect. The
        // production authoring seam does not implicitly author the std package,
        // so publish the compiler-owned std artifact into the same canonical
        // store before compiling the user package.
        seed_official_std_package(&platform_sources, artifact_root.path())
            .expect("seed canonical std package into the Phase 4 artifact store");
        let authoring_receipt = match build_authoring_object(
            ...
            Err(error) => {
                ...
                return Phase4FixtureBuild::Rejected {
        ...
        Phase4FixtureBuild::Published(Self {
            artifact_root,
            authoring_receipt,
            package_ref,
            package_artifact,
    ...
    pub(in crate::host::request_entry) fn open_store(&self) -> CanonicalArtifactStore {
        CanonicalArtifactStore::open(self.artifact_root.path()).expect("open canonical store")
    }

    pub(in crate::host::request_entry) fn package_ref(&self) -> &PackageArtifactRef {
        &self.package_ref
    }
    ...
    fn http_header(
        ...
        BytecodeRequestStartFrameHeader {
            ...
            http_request: BytecodeHttpRequestFrameHeader {
                method: "POST".to_string(),
                url: format!("http://phase-4.example.test{}", self.ingress_path),
                path: self.ingress_path.to_string(),
                query: Vec::new(),
                headers: Vec::new(),
            },
    ...
    pub(in crate::host::request_entry) fn canonical_request(
        ...
        self.canonical_request_with_deadline(correlation, mode, request_body, None)
    }

    /// Canonical request with an explicit wire deadline whose `expires_at` is
    /// `timeout_ms` in the future. The deadline race negative pairs a
    /// near-term deadline with a far-future sleep so the deadline is the only
    /// reachable terminal.
    pub(in crate::host::request_entry) fn canonical_request_with_deadline(
        &self,
        correlation: &Correlation,
        mode: &str,
        request_body: &[u8],
...
pub(in crate::host::request_entry) fn error_chain_text(
