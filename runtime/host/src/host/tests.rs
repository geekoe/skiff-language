...
async fn package_test_spawn_worker_claims_with_activation_and_rejects_mismatched_descriptor() {
    ...
    let (fail_header, payload): (SpawnFailRequestFrameHeader, Vec<u8>) =
        decode_typed_binary_frame(&router_binary(fail_request))
            .expect("spawn.fail.request should decode");
    assert!(payload.is_empty());
    let error_text = fail_header
        .diagnostics
        .as_ref()
        .and_then(|diagnostics| diagnostics.get("error"))
        .expect("spawn fail diagnostics should include activation mismatch")
        .to_string();
    assert!(
        error_text.contains("activationIdentity"),
        "unexpected diagnostics: {error_text}"
    );
    ...
    let outcome = claim_once
        .await
        .expect("spawn worker should finish failed mismatched-activation claim");
    assert!(matches!(
        outcome,
        super::spawn_worker::ClaimOutcome::Claimed
    ));
