...
pub(crate) const ACTOR_BOOTSTRAP_ENCODING_V1: &str = "skiff-canonical-v1";
...
pub struct AdmittedActorOwnerFence {
    ...
    pub epoch: u64,
    /// Exact deployment build pinned by the admitted incarnation.
    pub build_id: String,
...
/// Narrow adapter seam for the concrete Actor executor. The host handoff does
/// not depend on its scheduler or field-frame implementation.
...
pub fn prepare_admitted_actor_execution(
    ...
    validate_build_id(&input.owner_fence.build_id, "owner fence build id")?;
...
fn validate_build_id(value: &str, label: &'static str) -> Result<(), ActorMethodAdmissionError> {
    let valid = value
        .strip_prefix(DEPLOYMENT_ARTIFACT_IDENTITY_PREFIX)
    ...
    if valid {
        ...
        Err(ActorMethodAdmissionError::InvalidBuildId { label })
    }
}
...
pub enum ActorMethodAdmissionError {
    ...
    #[error("{label} must be an exact deployment artifact identity")]
    InvalidBuildId { label: &'static str },
...
mod tests {
    ...
    fn build_id() -> String {
        identity(DEPLOYMENT_ARTIFACT_IDENTITY_PREFIX, 'f')
    }
    ...
    #[test]
    fn actor_create_bootstrap_is_retained_with_exact_fence_build() {
        ...
        assert_eq!(request.owner_fence.build_id, build_id());
        assert_eq!(
    ...
    #[test]
    fn wrong_build_id_fails_closed() {
        let mut input = admitted();
        input.owner_fence.build_id = "skiff-service-deployment-v2:sha256:old".to_string();
        assert_eq!(
            prepare_admitted_actor_execution("runtime-1", input).unwrap_err(),
            ActorMethodAdmissionError::InvalidBuildId {
                label: "owner fence build id"
            }
        );
    }
    ...
    fn every_external_identity_must_match_the_owner_fence() {
        ...
        let mut input = admitted();
        input.owner_fence.actor_implementation_identity =
            ActorImplementationIdentity::new(identity("skiff-actor-implementation-v1:sha256", 'e'));
        assert_eq!(
            prepare_admitted_actor_execution("runtime-1", input).unwrap_err(),
            ActorMethodAdmissionError::ActorImplementationMismatch
        );
    ...
    #[tokio::test]
    async fn actor_sync_method_calls_only_the_dedicated_executor() {
