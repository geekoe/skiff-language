...
use skiff_runtime_model::{
    ...
    runtime_value::{CallbackCapabilityCarrier, HeapNode, RuntimeValue},
};
...
fn program_context(interpreter: &Interpreter) -> ProgramExecutionContext<'static> {
    ...
    ProgramExecutionContext::new(ProgramExecutionInput {
        ...
        runtime_activation: Arc::new(RuntimeActivation {
            service: ServiceMeta {
                id: "skiff.run/callback-owner-test".to_string(),
                display_name: None,
...
async fn callback_native_method_error_and_cancel_release_guard_without_rollback() {
    for error in [
        ...
        let outcome = wait
            ...
            .await;
        let mut caller_heap = RequestHeap::default();
        assert!(completed(outcome, ContractTypeRef::builtin("string"))
            .finalize(&mut caller_heap)
            .is_err());
