...
fn adapt_ingress_arguments(
    ...
    if request.payload_encoding() != RequestPayloadEncoding::RuntimeBinary {
        return Err(RuntimeError::InvalidArtifact(
            "canonical assembly ingress does not accept recoverable spawn payloads".to_string(),
        ));
    }
...
mod tests {
    ...
    async fn ingress_hands_fixed_failure_up_without_importing_an_external_caller() {
        for kind in [
            ...
            let fixture =
                ServiceErrorConsumerFixture::new(kind, ConsumerTopology::OneHop, true, false);
