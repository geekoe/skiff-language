...
fn package_typed_throw_clones_the_exact_carrier_into_the_request_heap_without_service_export() {
    ...
    let payload =
        RuntimeValueCarrier::identified(RuntimeValue::Heap(payload_handle), outer_identity.clone());
...
fn package_throw_rejects_a_service_runtime_failure_without_exporting_it() {
    ...
    registry.register(
        ...
        RegisteredTestEffect {
            ...
            step_expect: None,
            outcome: RegisteredTestEffectOutcome::Throw(RegisteredTestEffectThrow {
                failure: RegisteredTestEffectFailure::ProviderFailure(RuntimeError::Unsupported(
                    "provider-only detail".to_string(),
                )),
                setup_heap: RequestHeap::default(),
...
fn record_array_and_bytes_snapshots_survive_setup_heap_destruction() {
    ...
    let response_plan = RuntimeTypePlan::synthetic_request_record(vec![
        RuntimeRecordFieldPlan::new("label", leaf_plan("string", RuntimeTypeNode::String), true),
    ...
    let HeapNode::Object(response) = dispatch_heap.get(*response_handle).expect("response") else {
    ...
    let RuntimeValue::Heap(payloads_handle) = response.fields().get("payloads").expect("payloads")
    else {
        panic!("payloads should be a dispatch-heap array");
    };
    let HeapNode::Array(payloads) = dispatch_heap.get(*payloads_handle).expect("payloads") else {
