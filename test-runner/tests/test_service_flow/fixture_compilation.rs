...
mod tests {
    ...
    fn checked_in_test_services_compile_and_assemble_through_the_ordinary_service_flow() {
        ...
        for fixture_name in CHECKED_IN_TEST_SERVICE_FIXTURES {
            ...
            let service_api = project.service_api.as_ref().unwrap_or_else(|| {
                panic!("{fixture_name} omitted its ordinary service projection")
            });
            assert!(
            ...
            let websocket_ingress = deployment
                ...
                .unwrap_or_else(|| {
                    panic!("{fixture_name} omitted its external WebSocket projection")
                });
