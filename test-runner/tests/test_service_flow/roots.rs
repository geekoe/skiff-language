...
mod tests {
    ...
    fn ordinary_test_service_resolves_public_private_and_test_local_roots_together() {
        ...
        let cases =
            discover_test_service_cases(&service, &service, false).expect("discover test case");
        let fixture =
            assemble_test_service_fixture(&project, &cases, Default::default(), "skiff-test")
                .expect(
                    "root-resolved test service must assemble through the ordinary service path",
                );
    ...
    fn runner_owned_private_gateway_rejects_missing_wrong_signature_and_public_leak() {
        ...
        let error =
            assemble_test_service_fixture(&missing, &cases, Default::default(), "skiff-test")
        ...
        let error = assemble_test_service_fixture(
            &wrong_signature,
            &cases,
            Default::default(),
            "skiff-test",
