...
"#,
    ...
    fn generated_service_stream_contract_compiles_through_consumer_file_ir() {
        ...
        validate_runtime_server_stream_item_plan(item_value_plan).expect(
            "a projected service stream item must satisfy the runtime Provider/Stream lane",
        );
        assert!(matches!(
            &operation.contract.stream,
...
"#,
        ...
        let error =
            compile_package_project_with_contract_dependencies(consumer.path(), &dependencies)
                .expect_err(
                    "an undeclared service alias must fail the package compile trust boundary",
                )
                .to_string();
        assert!(
            error.contains("for iterable must be Array, Stream, or Map"),
    ...
    fn explicit_definition_compiles_to_a_code_free_service_contract() {
        ...
        let contract =
            compile_service_contract(definition).expect("explicit contract should compile");
        validate_service_contract_identities(&contract)
            .expect("contract identities should be valid");
    ...
    fn protocol_identity_tracks_semantics_but_not_diagnostic_text() {
        ...
        changed.operations.get_mut("echo").unwrap().return_value.ty =
...
"#,
        ...
        let provider_project = compile_package_project_with_contract_dependencies(
            provider.path(),
...
"#,
        ...
        let consumer_project = compile_package_project_with_contract_dependencies(
            consumer.path(),
  ...
  return payments/echo(input)
            ...
            let error =
                compile_package_project_with_contract_dependencies(temp.path(), &dependencies)
                    .expect_err(
                        "invalid contract source must fail the package compile trust boundary",
                    )
                    .to_string();
...
"#,
        ...
        assert!(
            error.contains(
                "dependency alias `payments` is declared by both a package and a contract"
            ),
