...
"#,
        ...
        let project =
            compile_package_project(temp.path()).expect("DB root reference should compile");
...
test "test-only root reference" {
  final missing: root.internal.missing.Helper = root.internal.missing.Helper { value: "hi" }
  assert true
}
"#,
        )
        ...
        let project =
            compile_package_project(ignored.path()).expect("test file should be excluded");
        assert!(project
            .package
            .file_ir_units
            .iter()
