...
pub(crate) fn package_type_names_from_file_units<'a>(
...
pub(crate) fn projection_visible_executable_signature(
    ...
    ExecutableSignatureIr {
        ...
        self_type: executable
            .self_type
            .as_ref()
            .map(|ty| projection_visible_type_ref(context_module, ty, package_type_names)),
        ...
        TypeDescriptorIr::Record { fields } => TypeDescriptorIr::Record {
            fields: fields
                ...
                .map(|(name, ty)| {
                    (
                        name.clone(),
                        projection_visible_type_ref(context_module, ty, package_type_names),
                    )
                })
                .collect(),
        },
...
pub(crate) fn projection_visible_type_ref(
    ...
    match ty {
        TypeRefIr::LocalType { type_index } => package_type_names
        ...
        TypeRefIr::Union { items } => TypeRefIr::Union {
            items: items
                .iter()
                .map(|item| projection_visible_type_ref(context_module, item, package_type_names))
                .collect(),
        },
