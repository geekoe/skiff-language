...
传统后端开发里，真正难的往往不是写一个函数，而是把 API、数据库、后台任务、配置、测试、
发布和运行时行为拼成一个可靠系统。AI 也最容易在这些隐含规则上出错：忘记事务边界、误用
队列、漏掉测试替身、写出不可恢复的后台任务、让接口和数据库 schema 漂移。

Skiff 把数据库、队列、服务调用、配置、日志、测试和热部署这些后端基础设施做成语言的一部分。
AI 写代码时面对的不是一堆框架、SDK 和运维约定，而是一套可以被编译器、测试工具和运行时共同
检查的服务模型。

## 核心价值

- **AI 更容易一次写对。** 服务 API、数据模型、后台任务、配置读取、测试替身和发布边界都有明确语法和规则，减少靠经验猜框架行为。
- **开发者专注业务。** 数据库、队列、服务调用、流式响应、日志和测试都内置在语言模型里，不需要为每个服务重复搭胶水层。
- **后端边界可检查。** 跨服务调用、持久化数据、异步任务参数、流式输出和外部调用都有明确边界，出错时明确失败。
- **本地开发到热部署是一套模型。** 服务编译后加载到运行环境，更新服务产物即可重新生效，不必为每个服务重启一套基础设施。
- **适合多服务和长期演进。** API、schema、依赖和发布身份由语言管理，减少服务之间“看起来兼容、实际不兼容”的情况。
...
这些能力如果都靠库和框架拼接，AI 需要同时理解很多隐含约定。Skiff 的做法是把它们收敛进语言：
代码描述业务，语言描述边界，平台负责运行。
