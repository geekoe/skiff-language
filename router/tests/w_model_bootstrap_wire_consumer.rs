//! M-bootstrap-wire consumer gate: skiff-router consumes the frozen
...
fn corpus_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../runtime/transport/testdata/router-rust-bootstrap-wire-corpus.json")
}
...
mod tests {
    ...
    #[test]
    fn router_consumer_freeze_checks_bootstrap_wire_corpus() {
        let corpus = corpus();
        assert_eq!(
    ...
    #[test]
    fn router_consumer_decodes_and_roundtrips_bootstrap_frames_strictly() {
        let corpus = corpus();
    ...
    #[test]
    fn router_consumer_enforces_empty_bootstrap_payload() {
        let corpus = corpus();
    ...
    #[test]
    fn router_consumer_builds_bootstrap_from_captured_epoch_source() {
        let corpus = corpus();
        let canonical = corpus["frames"]
            .as_array()
            .expect("frames must be an array")
