...
use skiff_router::ws::{
    AcquireDecision, AllowAnyPendingAdmission, ClientConnectionIndexOptions, LedgerOptions,
    NoopNotificationObserver, OverflowPolicy, PendingAdmissionSender, RuntimeGenerationPinLedger,
    WebSocketLane, WebSocketLaneOptions, WebSocketRequestBrokerOptions,
};
...
use ws_harness::{
    ...
    FakeRuntimePeer, FakeRuntimeSessionClose, FakeRuntimeViolationSink,
};
...
fn router_side_tuple(connection_id: &str, runtime: &RuntimeSessionEpoch) -> WebSocketGenerationLifecycleTuple {
    let mut tuple = pin_tuple(connection_id, &runtime.replica_id);
    tuple.router_session_id = format!("{}#{}", runtime.replica_id, runtime.connection_generation);
...
fn lane_with_pending_admission(
...
async fn real_order_acquire_ack_before_connect_settle_admit_register() {
    ...
    assert!(
        matches!(
            lane.ledger.handle_acquire(&runtime, &acquire_control("acquire-1", &acquire)),
            AcquireDecision::Ack(_)
        ),
