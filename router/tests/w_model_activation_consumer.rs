//! M-activation consumer gate: skiff-router consumes the frozen
//! C-model-activation corpus directly through the W-model transport codecs.
//!
//! The Router side of the activation transaction wire (plan §4.1,
//! C-model-activation §1/§2) encodes `prepare` / `commit` / `abort`
//! (RouterToRuntime) and decodes `prepared` / `reject` (RuntimeToRouter).
...
mod tests {
    ...
    #[test]
    fn router_consumer_roundtrips_activation_transaction_frames_through_w_model_codecs() {
    ...
    #[test]
    fn router_consumer_sees_all_frozen_activation_transaction_cases() {
