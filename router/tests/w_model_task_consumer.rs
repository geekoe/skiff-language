//! M-task consumer gate: skiff-router consumes the frozen C-model-task
//! corpus (`runtime/transport/testdata/task-wire/`) through the canonical
//! task codec (`callerKind` closed enum). The legacy old shape is rejected
...
mod tests {
    ...
    #[test]
    fn router_consumer_roundtrips_task_wire_corpus_through_canonical_codec() {
        let catalog = catalog();
    ...
    #[test]
    fn router_consumer_sees_all_frozen_task_scenarios() {
    ...
    fn expected_payload_presence(name: &str) -> &'static str {
        match name {
            "task.submit.request.function"
            | "task.submit.request.actorMethod"
            | "task.submit.request.legacy-no-caller-kind" => "required",
            "task.submit.response" | "task.submit.error.parentNotFound" => "empty",
            _ => panic!("unexpected task frame {name}"),
        }
    }
}
