...
- binary HTTP、typed JSON adapter、server stream、平台选中的WebSocket业务消息dispatch、普通runtime
  payload都必须被建模为`IngressMode`，而不是在一个大函数中互相读取局部变量。现有raw WebSocket
  receive只能视为待迁移的transport mode，不能据此冻结用户handler surface。
