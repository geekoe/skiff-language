# Durable Schema Evolution Draft

日期：2026-07-10

> 状态：草案，未实现。
>
> 本文保存未来设计输入，不是当前语言规范，也不是已经生效的长期架构契约。正文仍包含实现阶段、
> 验收和旧 `spawn` / `queue` 术语；进入实现前必须按当前 `dispatch`、DB 与 recoverable contract
> 重新审阅，并先更新对应 canonical 文档。

本文目标是把 DB object、DB recoverable-envelope
