...
- 说明普通数据、nominal object、native handle、`any I`（`carrier = Local`）如何进入同一恢复机制；以及跨 service
  `any I` 进恢复边界**第一版统一 fail-closed**，及其目标态（service callback transport 落地后）的直传回拨机制。
- 规定恢复后的等价语义：类型、接口投影和可恢复状态保持一致；heap 地址不保持。
- 给出 compiler/runtime 的边界分工：静态闭包检查 + encode 时动态 carrier 查询 + decode 时按版本化代码恢复。
- 说明 ToolProvider 不需要 provider key registry；`any ToolProvider` 能否跨边界由其可恢复性决定（本地可恢复，
  跨 service provider 第一版 fail-closed）。
...
- 保证无法加载的历史 artifact 也能运行；平台目标是 artifact/version 可寻址，缺失时 fail closed。
...
跨 service 行为/接口/native adapter 节点进恢复边界 = 第一版 fail-closed（卡 service callback transport 与 sealed payload）；
...
- concrete type identity 一致，或满足该 type 声明的兼容恢复规则。
- 对 `any I`，interface identity 与 method slot projection 一致。
- 可恢复 state 一致。
- 方法调用语义仍可用。
...
可恢复值按 envelope 携带的 code identity **定位写入时的那份实现**来恢复，而不是用恢复时的当前版本去重新解释旧值。
decode 找不到对应 artifact / linked image 时 fail closed（见 §Envelope）。因此“恢复”不包含“跨版本行为恢复”：
不存在“用新版本代码恢复旧版本写入的行为值”这一语义。

这条与 DB 普通字段的 schema 演化是两条正交的线：

- 行为值（nominal object、`any I` local carrier、有 durable adapter 的 native handle）的恢复，靠 code identity
  锁定写入时实现 + fail closed，不依赖也不触发 schema migration。
- DB 普通数据字段的类型变更、rename、backfill 属于 **DB schema migration**（`../reference/db.md §11` 当前未定），
  与可恢复 codec 无关。一个旧 row 里的普通字段能否在新版本读出，由 migration 策略决定，不由本文负责。

换言之：service database 与稳定 service id 绑定、不按 version / build 隔离（`../reference/db.md §9`），所以旧 row 被新版本
读取是常态；但本文只保证“能定位写入时实现就能恢复，否则 fail closed”，不承诺把旧行为值迁移到新实现。

### Artifact availability

`LocalCode` 恢复的前提不是“当前 runtime 里碰巧还有旧代码”，而是平台能按 envelope 中的 code identity 重新取得写入时的
linked image / artifact。只把 `build_id` 写进 envelope、却没有 artifact 可寻址存储与保留策略，是不完整实现。

第一版采用以下约束：

- 写入 recoverable behavior value 前，runtime/router 必须递归检查 recoverable tree 中所有需要代码的节点，并证明每个
  `artifact_identity + build_id` 可由 artifact store 加载；否则 encode 以稳定 code fail closed。
- `artifact_identity` 是 linked image / artifact store 的 canonical identity，由 service identity、artifact manifest identity 和
  build pipeline 输出共同决定；同一 build 可重现时必须稳定，不同 linked image 不得碰撞。`build_id` 标识具体 build bytes。
- dev 模式下，`~/.skiff/dev/artifacts` 或等价 artifact root 里的 build 也必须按 build id 可寻址；watch/reload 不能只保留
  “当前 build”并清掉仍被 DB/spawn/queue payload 引用的旧 build。
- production / 内网部署的 artifact 保留、GC 和诊断是平台职责。GC 不得删除仍可能被 service DB、spawn/queue payload 或
  persistent work item 引用的 build；无法证明引用消失时必须保守保留。
- 目标态跨 service sealed opaque payload 若被对端持有，也构成对构造侧 artifact/build 的潜在引用；sealed payload 的 expiry
  必须参与 artifact retention / GC 判定。这是 artifact/build 保留问题，不是 callback handle 或活实例 GC；平台没有发行记录或
  expiry 证明时必须按最大 TTL 或保守保留策略处理。
- decode 找不到 artifact 时 fail closed，但这是平台可诊断错误，不是应用 schema migration。错误信息必须包含 service id、
  artifact identity、build id 和 boundary kind。
...
enum RecoverableCodeIdentity {
    ...
    // B. 本地代码对象：NominalObject，以及 InterfaceValue.self_node 中的 concrete self。
    //    存“去哪找写入时实现来重建一个等价对象”——concrete payload 序列化在 node.state 里。
    LocalCode {
        artifact_identity: String,          // 写入时的 linked image / artifact 定位（承重，必填）
        build_id: String,                   // 复用既有 dynamicBuildId 投影，逐字节一致；decode 端按它对账，不新增第二份投影
        concrete_type_identity: String,     // 装箱源 / nominal object 的 concrete nominal instantiation identity
        package: Option<PackageCoordinate>, // 实现来自 package 时带 package 坐标
    },
...
- 原平铺草图里的 `type_identity` 收进 `LocalCode.concrete_type_identity`。
- `RecoverableNode` 是递归单位。plain data 节点通常 `code_identity = None`；需要代码恢复的本地对象节点使用
  `code_identity = LocalCode{...}`；durable native handle 节点使用 `code_identity = NativeAdapter{...}`。数组元素、map
...
- `RecoverableMapKey` 第一版只支持 `string` 或单一名义 representation over string，和当前 boundary map key 规则一致。
  该 representation 必须是不依赖版本化代码执行的 canonical string representation；若某种 key 规范化需要
  `LocalCode` 或 native adapter，第一版不能作为 recoverable map key。其它 primitive key domain 若未来进入语言，需要先扩展
...
- `NominalObjectState::DefaultFields` 表示默认结构恢复；`NominalObjectState::Custom` 表示 concrete type 自定义恢复，入口由
  `LocalCode.concrete_type_identity` 和 `restore_schema_version` 定位。自定义 durable state 仍是 `RecoverableNode`，可递归包含
  plain data 或其它可恢复值。
- **`any I` 的恢复机制不在本 enum 重新判，而是取自 `any I` 自己的 `InterfaceCarrier` 分支**
  ...
  分支。所以：
  - `carrier = Local` → `InterfaceValue` wrapper 节点 `code_identity = None`，interface/projection 写入
    `InterfaceValueState`，concrete self 的身份与状态写入 `self_node`（通常是 `NominalObject + LocalCode`）。
  - `carrier = Remote` → 是正向远程引用（consumer 主动调一个已发布 public instance），它是 request-scope 值、
    **不进恢复机制**（见 §Cross-Service Interface Value）；本 enum 不为它设分支。
- `value_kind` 仍区分 `NominalObject` 与 `InterfaceValue`：它决定恢复出的静态形态，与 code identity 的定位职责分离。
- decode 阶段必须能用每个节点自己的 `code_identity` 加载对应 linked image / artifact / native adapter；找不到时 fail closed，
  不做跨版本行为恢复（见 §Definition“不做跨版本恢复”）。
- `InterfaceValueState` 必须携带 interface identity、method projection identity 和 `self_node`。`self_node` decode 后必须得到
  concrete nominal object；native/custom resource 若要作为 interface self，必须被某个 nominal object 或该 nominal type 的自定义恢复
  封装。decode 恢复 concrete self 后，必须重新校验 concrete type 仍 implements 该 interface，并按 projection 重建 method
  table。projection identity 不从 source method name 临时推导。
...
- **可存储底线**：写入 DB 的值必须可恢复；不可恢复的 request-local resource、未声明恢复语义的 native value、`carrier = Remote`
  的行为引用等，写入时 fail closed。
...
- **recoverable-envelope lane**：静态类型图可能需要携带 code identity / carrier state / durable adapter state 才能恢复的
  **顶层 stored field**，例如字段类型就是 `any I`、字段是 nominal object value（默认结构恢复或自定义恢复，都需要
  `LocalCode` 定位写入时 concrete type）、字段内嵌 `any I` / behavior object / durable `NativeHandle`。该顶层 stored field
  整体存为显式 envelope。
...
1. 先按 `expected_type_plan` 做 envelope-level 预检：校验 `RecoverableNode.value_kind`、`variant_identity`、plain data shape、
   map key domain、nullable null/non-null shape 和 union branch identity 是否兼容。nominal object 的默认字段 identity / restore
   state 校验需要加载写入时 artifact 并取得旧 concrete restore plan 后执行，不能只靠当前 expected type 推断。
2. 对 `value_kind = InterfaceValue` 节点，expected type 必须是兼容的 `any I`。先校验
   `InterfaceValueState.interface_identity` 与当前 `I` 兼容、`method_projection_identity` 与当前 method slot projection 兼容。
   然后要求 `self_node` 是 `NominalObject + LocalCode`：按 `self_node.code_identity.{artifact_identity, build_id,
   concrete_type_identity}` 加载写入时 artifact，派生该 concrete type 的 restore plan，并恢复 concrete nominal object。最后校验
   该 concrete type 可以赋给当前 expected `any I`、仍 implements `I`，再重建 method table。
3. 对 `LocalCode` 节点，按该节点的 code identity 加载写入时 artifact，再校验恢复出的 concrete type 可以赋给当前
   expected type。`LocalCode` 分支不读取 `InterfaceValueState`；interface wrapper 的校验只发生在上一条。
4. 对 nominal object，先按写入时 concrete type 和 state 恢复等价值，再按当前 expected nominal/interface/union plan 做可赋值
   检查。当前 schema 不接受该旧 concrete type 或旧 record shape 时 fail closed；这不是自动 schema migration。
5. 对 DB 普通 schema-projectable 字段的跨版本变化，仍由 DB schema migration 负责；recoverable decode 只负责“当前 expected
...
- `build_id` 只定位写入时 artifact，不参与 type/interface identity 的相等判断。type/interface/field/union branch identity
  由 compiler 写入 artifact metadata，必须是 canonical、跨同一语义实体的 rebuild 稳定值。
- plain data：`value_kind`、nullable shape、primitive canonical domain 必须与 expected type plan 匹配；不做 string/number 等
  宽松 coercion。
- record/default nominal fields：field identity 必须逐项匹配写入时 concrete restore plan；字段 rename、删除、拆分、合并属于
  DB schema migration 或 custom restore version，不由 recoverable decode 自动猜测。
- nominal expected type：恢复出的 `LocalCode.concrete_type_identity` 必须等于当前 expected nominal identity；若 expected type 是
  interface，则按 interface 规则检查；若 expected type 是 union，则先按 union branch 规则选中分支。
- interface expected type：`InterfaceValueState.interface_identity` 必须等于当前 interface identity；第一版不做 interface alias。
  `method_projection_identity` 必须等于当前 projection identity。恢复出的 concrete type 还必须在写入时 artifact metadata 中实现该
  interface/projection；当前 expected plan 只校验 exact interface/projection identity 是否接受该 concrete type identity，不用当前
  artifact 重新解释旧实现。否则 `recoverable_interface_conformance_missing`。
- union expected type：若 envelope 带 `UnionBranch{ union_identity, branch_identity }`，两者必须与当前 expected union 和 branch
  identity 精确匹配。没有 branch identity 时，只允许 compiler 证明 payload shape 在当前 union 中唯一；否则
  `recoverable_expected_type_mismatch`。
- custom restore：`restore_schema_version` 只在写入时 artifact metadata 中精确定位 concrete type 的 restore plan。第一版不使用
  当前 artifact metadata 接收旧 durable state，也不做 restore schema migration；写入时 artifact 找不到精确 plan 时
  `recoverable_expected_type_mismatch` / `recoverable_artifact_unavailable`。
...
1. `trust_boundary != OwnerInternal` 的明文行为节点先失败，不加载 artifact。
2. envelope schema/value kind/variant/shape 不匹配报 `recoverable_expected_type_mismatch` 或 `recoverable_state_invalid`。
3. artifact/native adapter 缺失报 `recoverable_artifact_unavailable` / `recoverable_native_missing_adapter`。
4. artifact 加载后 interface/projection/impl 不成立报 `recoverable_interface_conformance_missing`。
...
- concrete type identity 进入当前 `RecoverableNode.code_identity`（需要代码恢复时为 `LocalCode.concrete_type_identity`）。
...
artifact metadata 必须为每个支持 custom restore 的 concrete type 写入：
...
- `restore_schema_version` 与 `LocalCode.concrete_type_identity` 一起定位 hook。找不到 plan、durable state 不符合 plan、hook
  输出对象 concrete type 不匹配时 fail closed。
- custom restore plan 属于写入时 artifact；decode 必须先加载写入时 artifact，再按该 artifact 的 plan 解释 durable state。
...
- encode：`value_kind = InterfaceValue`；`code_identity = None`；
  `state = InterfaceValueState{ interface, projection, self_node = recover(payload) }`。`self_node` 是 concrete self 的唯一身份源，
  通常是 `value_kind = NominalObject` 且 `code_identity = LocalCode{ artifact/build/concrete type/package }`。
...
- decode：先 decode `self_node` 得到 concrete nominal object → 校验它仍 implements I → 按 `method_projection_identity`
  重建 method table → 返回 `any I`。失败 fail closed（见 §Definition“不做跨版本恢复”）。

**`carrier = Remote`**（装箱源是已发布 public instance，如 `remoteLlm/llmInstance`）→ 是 consumer **主动调**一个远程
公开实例的 request-scope 引用，**不进恢复机制**（见下节）。若该值被写入 DB/spawn/queue/persistent payload 或任何显式
recoverable envelope slot，encode 必须以 `recoverable_remote_carrier_not_persistable` fail closed。
...
`carrier = Remote` 或 self 不可恢复时 fail closed。
...
- **正向：consumer 主动调远程公开实例。** `remoteLlm/llmInstance as I`——装箱出 `carrier = Remote`，consumer 持有它、
  **主动** consumer→callee 调用，复用现状 service dependency dispatch。这是 request-scope 引用，consumer **本地主动调**
  这个用法**不进恢复机制**：它不被持久化重建，只是“指向某个已发布远程实例”的坐标，由 `any-interface-value.md` 的
  `Remote` carrier 承载，本文不重复定义。但若把这个 `carrier = Remote` 值**当作 payload 传去对端 service 等回拨**，
  它既不带 self 字节（无法直传），也不是 durable remote handle，故以 `recoverable_remote_carrier_not_persistable`
  **第一版 fail-closed**——
  “不进恢复机制”仅指 consumer 本地主动调，不含把它当 payload 传出。
...
- **carrier 可执行性靠构造侧 service 级代码恢复**，不靠某个活实例：回拨到达时按字节里的 code identity（build id
  寻址）加载 linked image、从 state 重建。故任意一个构造侧 service 实例都能处理回拨，构造侧重启、扩缩容不影响——
  这正是 §Definition“heap 地址不保持、按 envelope 恢复等价值”的直接应用。
...
- **跨 service 直传 payload 必须 sealed**：payload 由构造侧生成并验证，至少绑定构造侧 service id、整个 recoverable tree
  中所有节点的 code identity（artifact/build/native adapter 等）、interface identity、目标 service id（或明确的 audience）、
...
字节结构和构造侧按字节重建的内部 codec 可以复用 `LocalCode` 恢复路径；但跨 service encode 第一版不开放。缺的不只是
...
- `DbPayload` / `SpawnPayload` / `QueuePayload` 是 owner service 内部的跨 request / 持久边界；`target_service = None`。
  `trust_boundary = OwnerInternal`。`carrier = Local` 且 self payload 全可恢复时允许；`carrier = Remote` 永远不是可恢复值，
  fail closed。`explicit_recoverable_slot` 对这类 owner-internal lane 不是 public ABI 开关；DB schema / spawn target /
  queue payload plan 本身就是可恢复边界。
...
- 第一版若 `trust_boundary != OwnerInternal` 且 recoverable tree 中出现 `value_kind = InterfaceValue`、`NominalObject`、
  `NativeHandle`，或任何 `LocalCode` / `NativeAdapter` 节点，encode 必须 fail closed；decode 收到这类明文行为 envelope
  或畸形行为节点也必须以稳定 code fail closed，除非未来 sealed opaque payload 生效且校验通过。`CrossService` 下
  `InterfaceValue` local carrier 的失败码是
  `cross_service_interface_callback_unavailable`；普通 `LocalCode` nominal object 或 `NativeAdapter` 节点的失败码是
  `cross_service_recoverable_behavior_unavailable`。`ExternalUntrusted` 下行为节点的失败码是
  `recoverable_untrusted_behavior_payload`。`carrier = Remote` 的失败码是 `recoverable_remote_carrier_not_persistable`。
...
- 目标态跨 service 行为值解除 fail-closed 前，必须同时具备 sealed opaque payload 与 service callback transport；否则不得把
  `LocalCode` state 作为明文发给对端 service。
...
artifact 不可加载、native adapter 缺失、interface conformance 失效、remote carrier 不可持久化、cross-service callback
缺失、cross-service behavior transport 缺失、不可信明文行为 envelope、sealed payload 校验失败等故障类别。
...
是 request-scope、不进恢复机制；跨 service 进恢复边界的 provider 第一版 fail-closed（见 §Cross-Service Interface
Value）。`HostProvider` 怎么恢复是 `HostProvider` 的责任，不是 agent 包的责任。
...
spawn payload 不保存 provider array。agent drain 类后台任务只能传 `threadId` / `runId` 等稳定 id；worker 进入新 request 后从
  ...
  recoverable boundary 中可恢复；`carrier = Remote` 或不可恢复 local self 在创建 run/config 时稳定拒绝。
...
- `any I` 是否可恢复、走哪条路径，取自其 `InterfaceCarrier` 分支（装箱点 `as I` 已定，recoverable 不重判）：
  `carrier = Local` 行为值走 `InterfaceValueState + self_node`（self 节点携带 concrete code identity）；
  `carrier = Remote`（正向引用已发布远程公开实例）是
  request-scope、不进恢复机制。跨 package（同 service 内）`any I` 在 package public 入口传参时是 request-scope 本地值；
  若进入 DB/spawn/queue/persistent payload，则仍按本文可恢复边界处理。**跨 service `any I` 进恢复边界第一版
  fail-closed**（卡 `any-interface-value.md §Evolution` 的 service callback transport）；坐标方案
  （含未发布内部 root 路径寻址）是演进方向、不在第一版。见 §Cross-Service Interface Value。
- 普通结构化值默认递归恢复；native/request-local 值默认拒绝；concrete type 可以提供自定义恢复或拒绝恢复。
- envelope 必须携带可加载的 code/artifact identity；decode 找不到对应实现时 fail closed。
- 平台必须提供 artifact/load-by-build-id 保留策略；否则不能把 behavior value 写入持久边界。
