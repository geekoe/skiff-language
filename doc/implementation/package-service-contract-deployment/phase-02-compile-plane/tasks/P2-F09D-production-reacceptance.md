...
在同时含T03H2、T03I、T03J、T04C与T04D的同一最终候选上，独立只读确认interface owner分类、resolved
expression exact projection、contract signature、File IR execution projection、public-instance handoff、
public-path normalization与PackageArtifact Local ABI已经收敛为单一production owner链。
...
- 输入任务：P2-T03F、P2-T03G、P2-T03H、P2-T03H1、P2-T03H2、P2-T03I、P2-T03J、P2-T04B、P2-T04C、P2-T04D。
...
- 确认expression exact projection不把`ResolvedTypeRef` debug/display重新当source syntax；LocalType identity与
  ContractTypeId在完整sidecar中分别保真，Map keys/for-in等派生表达式不丢projection。
...
- DAG：波次9l只读production复验；等待T03J合入既有T03H2/T04D候选，与R10I evidence refresh共同通过后
  解除T07恢复。
  风险：高。
- worktree：`/Users/geek/workspace/skiff-p2-production-reacceptance`；从同时含T03H2、T03J与T04D的同一integration
