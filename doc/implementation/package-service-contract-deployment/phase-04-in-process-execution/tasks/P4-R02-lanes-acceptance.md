...
首次验收在`ee1609c` FAIL，第二次在`9809dee`因stream terminal/drop ownership环FAIL，第三次在`484cab0`因pull
source error和file native consumer异常出口未清理FAIL。复验还必须阅读P4-F06/F07/F08/F09/F10合同与合流diff，
逐项确认canonical callback映射、
...
7. **STREAM_TERMINAL_DROP**：buffer满时End/Error publication不阻塞producer或丢失顺序；未消费、外部边界拒绝、
   pull source error、file native consumer异常退出及runtime/request drop均打破producer/runtime clone环，
   task/registry/lease/capability归零。
