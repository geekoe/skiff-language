...
- 风险/验收组：高风险 runtime linking boundary；R03对 T04、T05、T06合成一次 runtime-link batch只读验收。
- 当前成熟度：R02已验收 deployment/assembly，T04/T05为 loader/image checkpoints；完成后推进
  pre-admission candidate。
- 有效证据状态：本任务 clean commit叠加调度时 exact R02/T04/T05 integration checkpoint。任何上游 public API、
...
- 依赖：R02 PASS，且 T04、T05已合入 integration；消费真实 public API，不复制 loader/resolver/image owner。
- 解锁：R03；R03 PASS后解锁 T07、T08A、T08B。
...
4. candidate包含 immutable shared image、activation templates和 T04 hydrated immutable ServiceContract store；
   提供 `(ServiceContractRef, ContractOperationId)` typed lookup且 admission后仍保留 canonical descriptor/value
   plan。不实例化 Phase 04 `ActivationContext`，不执行 materialization/dispatcher/callback/cancel。
