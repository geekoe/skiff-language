...
- Each assigned shard owns exactly one surface:
  ...
  - `package-call-projection`: Skiff compiler/test-overlay projection only; resolve legal calls from
    exported test cases to published production-package callable facts so they do not acquire the
    six conservative unknown effects. Cover all six `aliyunoss` cases and all four `track` cases.
    Preserve fail-closed behavior for genuinely unresolved calls.
- Worktree: create a shard-specific worktree from the relevant integration repository
  ...
  `skiff-phase-05-integration` for package-call projection).
