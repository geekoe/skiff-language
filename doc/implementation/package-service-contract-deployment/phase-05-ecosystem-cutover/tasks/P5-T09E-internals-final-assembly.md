...
script/AGENTS接线。不得修改任何service contract/package/deployment/source、
...
2. workflow按`publish contracts -> compile packages independently -> validate deployments -> resolve one assembly`
   执行；任何missing/tampered/duplicate provider、Host collision或partial write不请求activation prepare。
...
7. `verify-phase05-ecosystem.mjs --non-live`去重调度T08/T10/T11/T12的affected checks、registry/provider/chat
   self-tests与五deployment closure；拒绝primary/stable reload路径，供T13唯一执行。
8. `scripts/run-phase05-ecosystem-probe.mjs --isolated --replicas 1`消费显式`SKIFF_ROOT`与
   `SKIFF_PACKAGES_ROOT`，用真实五service roots、temporary store/router/runtime/Mongo及fake upstream执行
   I03完成态；task只运行其self-test，不运行actual combined probe。
