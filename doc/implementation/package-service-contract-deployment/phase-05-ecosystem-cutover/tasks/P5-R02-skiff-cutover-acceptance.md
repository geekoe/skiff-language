...
T02–T05任务合同与各自证据，检查主Agent给出的exact clean merged commit/tree及P5-I02
combined integration probe ledger。
...
- runtime startup/control经production resolver/load/link/admit staged ACK与commit后注册；pre-commit reject/abort、request
...
- I02至少覆盖tooling产出的fixture经prepare/admit/commit被router/runtime加载、Host request到最终业务
  结果、candidate reject/abort保留旧committed generation及test-runner消费同一接口。
