...
使用未参与F16A/B/C、F18A/B/H/J、D20、I16或其它验收的全新独立只读Agent。权威设计：架构§3、§9、§10、§14。
...
- authoring/runner在首个manifest/source/store/dependency IO前guard；different-root typed zero-read且hostile artifact store路径
  从未创建，pipeline仅defense。必须复验R18A首次在`ecc53ec`提出、F18J关闭的同一pre-store顺序blocker。
...
不重跑F18A/B/H/J或I16矩阵、不跑Host/full suite。检查extra-review的第二reader/resolver与边界职责；候选、compiler
