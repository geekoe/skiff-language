...
- loader/admission在激活assembly前一次性完成：
  ...
  - 无递归环。
- Runtime不读取或绑定`PackageSchemaIndexIdentity`来重新证明公开性；否则无关Package类型会污染protocol。
  第一版public-only由schema record生成/发布入口保证，Runtime只接受canonical store中可按type id解析且通过
  内容身份校验的record。
...
- runtime loader聚焦测试覆盖真实filesystem records、共享record去重、缺失、额外、错hash/owner/key、
  contract未require、closure缺child及SCC。
