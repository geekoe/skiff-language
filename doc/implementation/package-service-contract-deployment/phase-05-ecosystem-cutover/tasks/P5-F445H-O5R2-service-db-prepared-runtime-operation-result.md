...
唯一 production `ServiceDbCapabilityStore` 已覆盖六个 prepared runtime 入口。同步 prepare 按旧
runtime入口的顺序完成 metadata、selector/query/order/projection校验和 write输入的 owned
BSON编码；find-many的 `limit: 0`在合法 query/order之后直接形成空计划，保持旧语义而不编译
projection。owned wait只保存 command、document、recoverable context、retention roots、
store/runtime owner等 caller-heap-free数据；one-shot finalizer才重新接收原 caller
`&mut RequestHeap`并物化结果。
...
| zero-limit validation-order correction | `dfe24f12` |
...
完成初始 production后，同一聚焦 selector执行 `10/10` GREEN、`102` filtered。

后续先增加
`zero_limit_skips_projection_compilation_and_provider_wait`：使用合法 query和合法 `title ASC`
order、故意传入未声明的 `missing.nested` projection，并要求空 completion/finalizer及 provider
start计数 `0`。修正前得到预期 RED：`10/11`通过、`1/11`失败、`102` filtered；prepare精确报
`db field path missing.nested is not declared on PreparedItem`。`dfe24f12`恢复旧校验顺序后，同一
selector最终执行 `11/11` GREEN。
...
- find-many：`limit: 0`在 binding → query filter → sort之后跳过 projection编译并保存为空计划；
  其他情况保存 owned `MongoFindManyPlan`；
...
对六条旧 runtime入口和新 command做了逐条只读顺序核对。两条 find-one仍分别保持
binding → key/query filter → sort（query入口）→ projection；create、update、replace的同步
selector/encoding步骤相对顺序也与旧实现一致。write同步步骤整体移到 provider wait之前是本
任务要求的 prepare边界，不是额外顺序漂移；除已修复的 find-many `limit: 0`外，没有发现第二
个同类机械差异。
...
- retention roots、Mongo command、lease assertion、immutable-file cascade和 implicit transaction
  finish/abort顺序沿用原实现；
- find-many空计划在 lease-live检查后直接生成空 completion，不启动 test provider或 Mongo
  provider，也不借用 active transaction session；
...
| `limit: 0`保持旧短路顺序 | 合法 query/order后跳过invalid projection；空finalizer且driver start保持 `0` |
...
| raw、mapping、lease/cascade helper无回归 | hermetic service-db suite除两个base限制外 `111/111` |
...
| service-db hermetic suite，显式排除下述两个base限制 | PASS：`111/111` unit，`0` doc |
...
在只排除该真实 Mongo测试后，suite实际执行 `112`项，结果为 `111` PASS / `1` FAIL。唯一失败是
未被本节点修改的
...
其余 `111`项 hermetic suite均已完成；在启动 O6/J1的 required full acceptance前，需要由对应
