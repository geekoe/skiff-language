...
> Status: active; revision 21; recovery epoch `r1`; architecture review is non-blocking; V5R implementation authorized; no implementation/proof lane complete
>
...
但不成为第二 lifecycle authority。slot 已整体 moved；compiler 在 source/MIR semantics中证明 single-consumer并发出
exact transfer/drop facts，atomic constructor只按这些 facts闭合 existing stack/control-flow references，无需新增
partial-state merge、stack-map语义或第二 lifecycle authority。
...
commit后的 clean integration HEAD 新建。架构/decision 文档不设置独立 review 或 PASS 前置门禁；实现直接以本节
authority、hard-cut 与 failure envelope 开码，并由 focused tests、Gate 和 frozen candidate semantic review验证。
...
D5 是本 revision 唯一 docs owner，exact write set见下表。该 docs commit只记录当前决定，不需要独立 Design
review或 PASS 才能创建 V5R code worktree；旧 `4cc8dd80` review结果仅是历史诊断材料。
...
D5 曾因此临时扩展为下表列出的 cross-document consistency cleanup。该 cleanup 保持 compiler-only source
semantics、single atomic image mint、construction-local bounded structural closure 与 checked runtime safe failure，
但不再是实现前置条件；未发现的外围文档措辞只记为非阻断债务，不继续扩大扫描范围。
...
尚未发生。这些修订是顺手收敛，不设置 review/PASS barrier，也不再阻塞 V5R。

### 2.21 User process ruling: architecture documents are not review gates

用户明确裁定：architecture、reference、decision、Phase Contract、MAP 与原则文档都不需要独立 Design review，
也不得以“文档完备性”或跨文档全量一致性阻塞生产实现。文档 owner 在开始实现前只需写清当前目标、核心 authority、
failure envelope 与写集；实现中发现真实冲突再就地 amendment。reviewer 不做开放集合的全仓文档扫描，外围措辞漂移
记为 non-blocking debt。

独立门禁只保留在 frozen implementation candidate：focused tests、canonical Gate、semantic implementation review
与 Acceptance 核对实际代码和证据。它们可以因第二 authority、fallback、unsafe failure、测试假绿或回归失败阻塞，
不能因未穷举同步无关架构文档而阻塞。本节 supersede §2.16–§2.20 中所有“fresh Design PASS before code”要求，
并同步修改 reusable principles、DEC1、Contract 与 runbook。
...
| --- | --- | --- | --- |
| D5 architecture/process alignment / non-blocking closeout | `codex/bcvm-p5-integration-r1` / `skiff-bcvm-p5-integration-r1` | `doc/architecture/{bytecode-vm.md,tail-call-execution.md,runtime-lazy-load-deployment.md,package-service-contract-deployment.md,db-capability-architecture.md,any-interface-value.md,observability-requirements.md,runtime-error-to-skiff.md}`；`doc/reference/{interface.md,any-interface.md,static-semantics.md,runtime.md,std-surface.md,observability.md,db.md}`；`doc/implementation/bytecode-vm-convergence/{README.md,runbook.md,large-change-execution-principles.md}`；`doc/implementation/bytecode-vm-convergence/decisions/dec1-executable-image-authority.md`；`doc/implementation/bytecode-vm-convergence/phases/phase-5-typed-host-effects-resources-streams.md`；`doc/implementation/bytecode-vm-convergence/tasks/phase-5-execution-map.md` | user architecture/process ruling；docs-only checkpoint；no review/PASS gate |
...
- Rolling join：本 rev21 process ruling/docs checkpoint → V5R hard cut + P5G selector companion（同一 atomic
...
- frozen candidate 的独立 semantic reviewer只判实际实现是否落实核心 Contract、已接受 invariant、第二
  authority/fallback/fail-closed，不审查 architecture 文档完备性；其后由另一名全新只读 Acceptance owner运行
  完整 Gate并核对 raw evidence。PASS 后才写 `results/phase-5.md`、合 main/push。
