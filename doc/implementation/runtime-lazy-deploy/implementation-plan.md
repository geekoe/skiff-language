...
handshake corpus 计数）保持。`assembly sync-state` 属 activation 协调层工具，已在 M4
随该层一起退役，不再作为恢复入口。
...
- 提供 CLI：`skiff release set/unset/get`，含幂等与冲突校验。
...
- 指针表的分片/高可用（当前单实例足够）。
- 线上预载提示的默认开关策略。
- 存量世代迁移与 activation 恢复工具回归（M4 已决策：无线上数据，`assembly sync-state`
  不回归）。
