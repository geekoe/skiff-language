...
- 批次文档：`doc/implementation/router-rust-migration-batch-4.md`（A3 节点条款、
  写边界、验证 owner）。
- A0 契约：`doc/implementation/router-rust-migration-a0-contract.md`（frozen，
  本节点只消费不修改）。
- A0 叶子：`doc/implementation/router-rust-migration-a0-leaf.md`。
- C-model-artifact 契约：`doc/implementation/router-rust-migration-c-model-artifact-contract.md`
  （strict reader boundary：不得绕过、不得路径猜测、不得复制 DTO/私有兼容层）。
- C-bootstrap 契约：`doc/implementation/router-rust-migration-c-bootstrap-contract.md`
