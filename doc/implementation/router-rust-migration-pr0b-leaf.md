...
- 批次文档：`doc/implementation/router-rust-migration-batch-3.md`（PR 0b 节点）。
- 权威设计：`doc/implementation/router-rust-migration-plan.md`（draft v5，
  重点 §5.2 C-net 与 PR 0b、§6.2(2) tooling 持续推进、§7 PR0b；冲突时以权威设计为准）。
- 直接父批次：`doc/implementation/router-rust-migration-batch-2.md`（M0 + C-net，已合入 main）。
- 冻结契约：
  - `doc/implementation/router-rust-migration-c-config-leaf.md`（C-config：唯一 Router
    process config schema/defaults/relative-path/redaction/unknown-key/golden corpus）。
  - `doc/implementation/router-rust-migration-c-net-contract.md`（C-net：Tokio multi-thread、
...
- `doc/implementation/router-rust-migration-pr0b-leaf.md`。
