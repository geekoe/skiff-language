...
- 批次文档：`doc/implementation/router-rust-migration-batch-5.md`（当前在
...
- 冻结契约：
  ...
  - `doc/implementation/router-rust-migration-c-spawn-contract.md`（resolver /
    `SpawnSubmitRouter` 模型边界，本节点只消费其 wire 前置，不实现 handler）。
  - 兄弟叶子：`doc/implementation/router-rust-migration-contracts-actor-leaf.md`
    （contracts 交付清单与 corpus 约定）。
- 先例：`doc/implementation/router-rust-migration-w-model-leaf.md`（batch 4
...
1. baseline 锚定：`main` = `85596193df24f1fb5d0745eabf049e7e1ebf5a79`；当前
   ...
   仅多 `doc/implementation/router-rust-migration-batch-5.md`（本叶子按路径引用）。
...
10. `doc/implementation/router-rust-migration-w-model-actor-spawn-leaf.md`（本文件）。
