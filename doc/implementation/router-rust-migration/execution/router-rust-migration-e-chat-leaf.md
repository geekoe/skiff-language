...
- 批次文档：`doc/implementation/router-rust-migration/execution/router-rust-migration-batch-10.md`（E-chat 节点）。
...
- `router/src/supervisor/session_ports.rs`（derived spawn recoverable args
  payload）
- `doc/implementation/router-rust-migration/contracts/router-rust-migration-c-session-contract.md`（deadline
  默认值与理由）
