...
- 批次文档：`doc/implementation/router-rust-migration/execution/router-rust-migration-batch-1.md`（C0-control 行、并行 ownership 边界、验证 owner）。
- 权威设计：`doc/implementation/router-rust-migration/execution/router-rust-migration-plan.md` §2.5（TS baseline cleanup 前置项）、§5.2（C0 为 C-net 硬前置）、§8（loop-risk live gates、TS test 删除 ledger 约定）、§10（loop-risk evaluator 与 health snapshot 语义）。
- 冲突时以权威设计为准。
...
5. 测试 inventory/ledger：仓库当前没有 router test ledger 文件；按权威设计 §9“每删除一个 TS test，
   ...
   `doc/implementation/router-rust-migration/execution/router-rust-migration-batch-1-test-ledger.md`。
...
- 测试 inventory/ledger：新建 `doc/implementation/router-rust-migration/execution/router-rust-migration-batch-1-test-ledger.md`。
- 本叶子文件。
