...
- 批次文档：`doc/implementation/router-rust-migration/execution/router-rust-migration-batch-1.md`（在 `integration/router-rust-migration-batch-1` 上，基线 main 分支尚未包含；本叶子按路径引用，集成合流后可用）。
- 权威设计：`doc/implementation/router-rust-migration/execution/router-rust-migration-plan.md`（draft v5，重点 §2.5 C0 / C-config；冲突时以权威设计为准）。
