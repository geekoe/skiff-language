...
`InterfaceValueState + self_node`；`self_node` 携带 concrete value 的 code identity 和可恢复 state。`carrier = Remote`
或 self payload 不可恢复时，边界操作 fail closed，且不得写入半截 DB row、不得提交 spawn/queue item。

跨 service 行为值第一版 fail closed。目标态需要 sealed opaque payload 与 service callback transport；这些能力落地前，
不得把明文 `LocalCode` / `NativeAdapter` / `InterfaceValue` state 发给对端 service 或 public client。离开 owner service
