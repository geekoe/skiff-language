...
adapter source/sink派生wire shape，例如typed HTTP body、query/path/header参数、HTTP response或typed
WebSocket message body。`pre`产生的内部context、guard内部值、WebSocket connection context及其它只在
