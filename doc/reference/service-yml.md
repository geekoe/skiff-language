...
- timeout、quota、state/resource binding及明确支持的service lifecycle value由所选
  `config.<profile>.yml`提供；例如`timeout: 30000`投影为可选`DeploymentPolicy.timeoutMs`，不在
  `service.yml`声明。并发是Router/Runtime连接级平台门禁，不是service lifecycle：
  `lifecycle.maxConcurrency`在任何service profile中都非法，也不进入ServiceDeployment。
